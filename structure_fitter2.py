...
def cmp_site(coords_list,site,frac_tol):
        
        return find_in_coord_list_pbc(coords_list,site,frac_tol)
...
def cmp_struct(s1,s2,frac_tol):
    ...
    for s1_coords, s2_coords in zip(s1, s2):
        ...
        for coord in s1_coords:
            ind = cmp_site(s2_coords,coord,frac_tol)
            ...
            if len(ind) > 1:
                ...
                avail[np.where(np.all(np.abs(fdist) == np.min(np.abs(fdist)), axis=1))[0]] = 0
...
def compare_structures(struct1, struct2, ltol = 0.2, atol = 5,primitive_cell=False,equivalent_species=[]):
