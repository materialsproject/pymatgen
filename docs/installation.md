...
2. `enum`: For the use of `pymatgen.transformations.advanced_transformations.EnumerateStructureTransformation` and `pymatgen.command_line.enumlib_caller` module. Gus Hart's enumlib provides a robust way to enumerate derivative structures. It can be used to completely enumerate all symmetrically distinct ordered structures of disordered structures via EnumerateStructureTransformation. Many other advanced transformations (e.g., MagOrderingTransformation) use EnumerateStructureTransformation. The enum.x and makestr.x executables must be in the path. Two implementations provide `enum.x`:
   - [Enumlib.jl](https://github.com/glwhart/Enumlib.jl) (recommended): a from-scratch Julia reimplementation of enumlib by one of its original authors. Its `enum.x` is a drop-in replacement for the Fortran executable: the `struct_enum.in`/`struct_enum.out` file contract is unchanged and pymatgen's enumlib test suite passes against it. Compared to the Fortran implementation, it is faster, better maintained, installs through the Julia package manager instead of requiring a Fortran toolchain and source compilation, and comes with significantly improved documentation at <https://glwhart.github.io/Enumlib.jl/dev/>. You will still need `makeStr.py` from the original enumlib repo in your path (a plain Python script, no compilation required).
   - The original Fortran [enumlib](http://github.com/msg-byu/enumlib): follow the instructions in its README to compile enum.x and makestr.x, or use `pmg config --install enumlib` (see below). Note that `EnumlibAdaptor` emits an advisory `UserWarning` recommending Enumlib.jl when it detects the legacy Fortran `enum.x`; enumeration behavior is unchanged either way.
...
If you would like to use the enumeration capabilities powered by Gus Hart's `enumlib`, the recommended route is [Enumlib.jl](https://github.com/glwhart/Enumlib.jl), a from-scratch Julia reimplementation by one of enumlib's original authors that provides `enum.x` as a drop-in replacement for the Fortran executable. It is faster, better maintained, installs through the Julia package manager (no Fortran toolchain or source compilation needed), and has significantly improved documentation at <https://glwhart.github.io/Enumlib.jl/dev/>. See the [Optional non-Python programs](#optional-non-python-programs) section above for details.
