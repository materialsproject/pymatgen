...
2. `enum`: For the use of `pymatgen.transformations.advanced_transformations.EnumerateStructureTransformation` and `pymatgen.command_line.enumlib_caller` module. Gus Hart's enumlib provides a robust way to enumerate derivative structures. It can be used to completely enumerate all symmetrically distinct ordered structures of disordered structures via EnumerateStructureTransformation. Many other advanced transformations (e.g., MagOrderingTransformation) use EnumerateStructureTransformation. The enum.x and makestr.x executables must be in the path. Two implementations provide `enum.x`:
   - [Enumlib.jl](https://github.com/glwhart/Enumlib.jl) (recommended): a from-scratch Julia reimplementation of enumlib by one of its original authors. Its `enum.x` is a drop-in replacement for the Fortran executable: the `struct_enum.in`/`struct_enum.out` file contract is unchanged and pymatgen's enumlib test suite passes against it. As of v0.3.4, Enumlib.jl ships all three enumlib executables (`enum.x`, `makestr.x` and `polya.x`). The easiest install is the pre-compiled standalone binaries on the [releases page](https://github.com/glwhart/Enumlib.jl/releases), available for Linux x86_64, macOS arm64 and Windows x86_64, each with a `.sha256` checksum. These bundle the Julia runtime (roughly 300-450 MB compressed per platform), so no Julia installation is required; simply untar and put the `bin/` directory on your PATH. For example, on Linux x86_64:

     ```shell
     curl -sSL https://github.com/glwhart/Enumlib.jl/releases/download/v0.3.6/enumlib-jl-0.3.6-linux-x86_64.tar.gz | tar xz
     export PATH="$PWD/enumlib-jl-0.3.6-linux-x86_64/bin:$PATH"
     enum.x --version  # verify: prints "enum.x (Enumlib.jl) 0.3.6"
     ```

     Substitute `macos-aarch64` or `windows-x86_64` (and the current version) as appropriate. No `LD_LIBRARY_PATH`/`DYLD_*` setup is needed: the executables resolve their bundled libraries relative to their own path. macOS note: the binaries are not yet code-signed or notarized, so Gatekeeper may block libraries downloaded through a browser (the failure can surface as a misleading "library not found" error). Downloading with `curl` or `wget` as above avoids the quarantine flag entirely; if you did use a browser, clear it with `xattr -dr com.apple.quarantine enumlib-jl-0.3.6-macos-aarch64` (no sudo needed). Alternatively, Enumlib.jl can be installed through the Julia package manager. Compared to the Fortran implementation, it is faster, better maintained, requires no Fortran toolchain or source compilation, and comes with significantly improved documentation at <https://glwhart.github.io/Enumlib.jl/dev/>. Known caveat: the bundled `makestr.x` fails on some structures with an `eps_dset` tolerance error (observed with v0.3.4; pymatgen's LiFePO4 enumeration test triggers it, and no fix is recorded in the Enumlib.jl changelog through v0.3.6), whereas pymatgen's full enumlib test suite passes with Enumlib.jl's `enum.x` paired with `makeStr.py` from the [Fortran enumlib repo](http://github.com/msg-byu/enumlib) (a plain Python script, no compilation required). pymatgen prefers `makestr.x` over `makeStr.py` when both are in the path, so if you hit this error, expose only `enum.x` from the bundle (e.g. via a symlink into a directory on your PATH) and use `makeStr.py` for structure generation.
   - The original Fortran [enumlib](http://github.com/msg-byu/enumlib): follow the instructions in its README to compile enum.x and makestr.x, or use `pmg config --install enumlib` (see below). Note that `EnumlibAdaptor` emits an advisory `UserWarning` recommending Enumlib.jl when it detects the legacy Fortran `enum.x`; enumeration behavior is unchanged either way. Enumlib.jl identifies itself via `enum.x --version` from v0.3.3 onward; older Enumlib.jl versions are misidentified as the legacy Fortran and will trigger the warning. (The v0.3.3 and v0.3.4 binaries misreport the version number itself as `nothing`, but the identifying `Enumlib.jl` token is present, so detection is unaffected; the number is reported correctly from v0.3.5.)
...
If you would like to use the enumeration capabilities powered by Gus Hart's `enumlib`, the recommended route is [Enumlib.jl](https://github.com/glwhart/Enumlib.jl), a from-scratch Julia reimplementation by one of enumlib's original authors that provides `enum.x` as a drop-in replacement for the Fortran executable (v0.3.4+ also ships `makestr.x` and `polya.x`). Pre-compiled standalone binaries for Linux x86_64, macOS arm64 and Windows x86_64 are available on the [releases page](https://github.com/glwhart/Enumlib.jl/releases); they bundle the Julia runtime, so no Julia (or Fortran) installation is needed. Download the tarball for your platform, untar it, and put its `bin/` directory on your PATH. See the [Optional non-Python programs](#optional-non-python-programs) section above for a worked example, a macOS Gatekeeper note, and a known caveat with the bundled `makestr.x`.

Alternatively, to compile the legacy Fortran `enumlib` from source, or to perform Bader charge analysis powered by the Bader analysis code of the Henkelmann group, use the pmg command line tool as follows:
