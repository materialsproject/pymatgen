...
- PR #133 `StructureMatcher.group_structures` gains a `symmetric` argument (default `False`, ignored with `anonymous=True`) that is passed through to `fit()`. Because `fit` is directional, greedy grouping previously depended on input order; `symmetric=True` removes that directional dependence (used by `MaterialsProjectDFTMixingScheme` to fix `PYTHONHASHSEED`-dependent phase diagrams, materialsproject/pymatgen#3113). (by @kavanase)
- PR #136 `ComputedEntry.correction` and `correction_uncertainty` no longer construct `UFloat` objects (`math.fsum` of adjustment values and a `math.hypot` quadrature sum of uncertainties instead), eliminating the `uncertainties>=3.2` "std_dev==0" `UserWarning` re-fired on every `.energy` access for Materials Project r²SCAN entries carrying a zero-uncertainty mixing adjustment, and making `ComputedEntry.energy` ~20× faster (noticeable in `PhaseDiagram` construction). `reaction_calculator` compares on `nominal_value` instead of falling through to the deprecated `AffineScalarFunc.__lt__`. (by @kavanase)
- PR #131 Fix `VolumetricData.write_file` corrupting CHGCARs whose `data_aug` is in the legacy list-of-lines format (lines carrying their own trailing `\n`): `write_aug` doubled the newlines, producing blank lines inside augmentation blocks that VASP rejects with `RD_RHO_PAW: ... data corrupt` / `PAW occupancies are missing on CHGCAR` under `ICHARG=1`. Regressed in 2026.5.17; output is byte-identical for already-stripped lines. (by @hanaol)
- PR #129 `PhononDos.cv`, `entropy`, `internal_energy` and `helmholtz_free_energy` no longer return `nan` when the frequency grid samples ω = 0 exactly (routine for phonopy `--dos-range` output and `np.linspace(0, f_max, n)` grids). The Bose-Einstein weights are evaluated only for ω > 0 with the analytic ω → 0 limit substituted for `cv` (→ 1) and `internal_energy` (→ 2k_BT); for `entropy`/`helmholtz_free_energy` the integrable logarithmic divergence at the zero sample is dropped, with a warning if `g(0) > 0` (outside the harmonic-lattice model). Grids not sampling ω = 0 are unchanged. (by @Divide-By-0)
- PR #127 Correct `VolumetricData` interpolation to use periodic `i / n` grid locations, including interpolation across cell boundaries. **Behavior change:** `value_at` and `linear_slice` now return corrected values and wrap fractional coordinates periodically instead of raising outside the unit cell. The callable `interpolator` is now a method rather than a `RegularGridInterpolator` attribute, so its former `.grid`/`.values` attributes and `method`/`fill_value` call options are no longer available. (by @Krataios14)
- PR #126 `Oszicar` restores exponent markers VASP occasionally omits for very small values (e.g. `-.302333-128` for `-.302333E-128`) in both electronic and ionic steps before conversion, instead of losing the value or raising. Standard notation such as `E+04` is unaffected. Fixes materialsproject/pymatgen#3699. (by @Krataios14)
- PR #125 `Strain` accepts a six-element Voigt vector as documented (previously raised a rank error in `SquareTensor`), routing through `from_voigt` so the factor-of-two shear scaling is handled. Fixes materialsproject/pymatgen#4549. (by @Krataios14)
- PR #124 `PotcarSingle` summary-statistics validation uses a magnitude-relative tolerance (`|a-b| / max(|a|, |b|, 1)`), so 1–2 ULP JSON round-trip differences in ~1e12-sized `VAR` values for heavy elements no longer mark a valid POTCAR as invalid. (by @matthewkuner)
- PR #128 Add `COMPATIBILITY.md` documenting breaking changes (the `pymatgen-core` counterpart of the retired `docs/compatibility.md`); `invoke update-changelog` now collects `[breaking]`-titled PRs into it at release time. Closes #120. (by @DanielYang59)
- PR #130 Fix a mypy error in `Vasprun` (`data_keys` typing). (by @Sanftperlig)
- `EnumlibAdaptor` writes symmetry-idealized fractional coordinates to `struct_enum.in`: sites in the same crystallographic orbit (at `symm_prec`) are group-averaged over their space-group images so they exactly obey the declared symmetry. Relaxed structures typically break it by ~1e-4 Å, which Enumlib.jl's `makestr.x` (unlike the legacy Fortran tool) rejects during its internal symmetry consistency check. `makestr.x` stderr is now captured and an `EnumError` is raised when it produces fewer structures than the enumeration reported, instead of silently returning a truncated list. The module docstring notes that Enumlib.jl now provides both `enum.x` and `makestr.x`. (by @shyuep)
