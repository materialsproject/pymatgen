...
class CompleteDosTest(unittest.TestCase):
    ...
    def test_get_dos_fp(self):
        # normalize=True
        dos_fp = self.dos.get_dos_fp(type="s", min_e=-10, max_e=0, nbins=56, normalize=True)
        ...
        self.assertAlmostEqual(sum(dos_fp.states * bin_width), 1, delta=0.001)
        # normalize=False
        dos_fp2 = self.dos.get_dos_fp(type="s", min_e=-10, max_e=0, nbins=56, normalize=False)
        bin_width2 = np.diff(dos_fp2.energies)[0][0]
        self.assertAlmostEqual(sum(dos_fp2.states * bin_width2), 7.279303571428509, delta=0.001)
        # binning=False
    ...
    def test_get_dos_fp_similarity(self):
        dos_fp = self.dos.get_dos_fp(type="s", min_e=-10, max_e=0, nbins=56, normalize=True)
        dos_fp2 = self.dos.get_dos_fp(type="tdos", min_e=-10, max_e=0, nbins=56, normalize=True)
        ...
        dos_fp = self.dos.get_dos_fp(type="s", min_e=-10, max_e=0, nbins=56, normalize=True)
        dos_fp2 = self.dos.get_dos_fp(type="s", min_e=-10, max_e=0, nbins=56, normalize=True)
        similarity_index = self.dos.get_dos_fp_similarity(dos_fp, dos_fp2, col=1, tanimoto=True)
