...
from pymatgen.entries.computed_entry import ComputedStructureEntry
...
class DefectEntry(MSONable):
    ...
    def __repr__(self):
        output = ["DefectEntry {} - {}".format(self.entry_id,
                                                 self.defect.name),
                  "Energy = {:.4f}".format(self.energy),
                  "Correction = {:.4f}".format(np.sum(self.correction.values())),
                  "Parameters:"]
