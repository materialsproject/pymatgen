...
class ChemEnvConfig():
    ...
    def __init__(self, materials_project_configuration=None, icsd_database_configuration=None, package_options=None):
        self.materials_project_configuration = materials_project_configuration
        self.icsd_database_configuration = icsd_database_configuration
    ...
    def setup(self):
        while True:
            ...
            if self.has_icsd_database_access:
                print(' - Access to icsd database is configured (add test ?)')
            else:
                print(' - No access to icsd database')
            ...
            print(' <2> + <ENTER> : setup of the access to an icsd database')
            print(' <3> + <ENTER> : configuration of the package options (strategy, ...)')
            ...
            if test == '1':
                ...
                self.setup_icsd_database_configuration()
            elif test == '3':
    ...
    def setup_icsd_database_configuration(self):
        self.icsd_database_configuration = {}
        server = raw_input('\nEnter MongoDB server (e.g. "mongodb.google.com") : ')
        database = raw_input('Enter database name (e.g. "icsd_data") : ')
        collection = raw_input('Enter collection name (e.g. "icsd_2014") : ')
        auth_name = raw_input('Enter database authentication name (e.g. "johndoe") : ')
        auth_password = raw_input('Enter database authentication password (e.g. "johndoepassword") : ')
        self.icsd_database_configuration = {'server': server,
                                            'database': database,
    ...
    @property
    def has_icsd_database_access(self):
        return self.icsd_database_configuration is not None
    ...
    def save(self, root_dir=None):
        ...
        config_dict = {'materials_project_configuration': self.materials_project_configuration,
                       'icsd_database_configuration': self.icsd_database_configuration,
    ...
    def auto_load(cls, root_dir=None):
        ...
        try:
            ...
            return ChemEnvConfig(materials_project_configuration=config_dict['materials_project_configuration'],
                                 icsd_database_configuration=config_dict['icsd_database_configuration'],
