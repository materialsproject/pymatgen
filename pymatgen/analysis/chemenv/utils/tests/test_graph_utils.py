...
class GraphUtilsTest(PymatgenTest):
    ...
    def test_get_delta(self):
        ...
        self.assertTrue(np.allclose(get_delta(n1, n2, edge_data), [-2, -6, -4]))
        with self.assertRaisesRegex(ValueError, 'Trying to find a delta between two nodes with an edge '
    ...
    def test_simple_graph_cycle(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex='SimpleGraphCycle is not valid : Duplicate nodes.'):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex='SimpleGraphCycle is not valid : '
                                                               'Duplicate nodes.'):
            SimpleGraphCycle([0, 2, 4, 6, 2])

        # Check the validate method
        # Nodes not sortable
        sgc = SimpleGraphCycle([FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(2)],
                               validate=False, ordered=False)
        self.assertFalse(sgc.ordered)
        self.assertEqual(sgc.nodes, (FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(2)))
        sgc.validate(check_strict_ordering=False)
        with self.assertRaisesRegex(ValueError, expected_regex='SimpleGraphCycle is not valid : '
                                                               'The nodes are not sortable.'):
            sgc.validate(check_strict_ordering=True)

        # Empty cycle not valid
        sgc = SimpleGraphCycle([], validate=False, ordered=False)
        with self.assertRaisesRegex(ValueError, expected_regex='SimpleGraphCycle is not valid : '
                                                               'Empty cycle is not valid.'):
            sgc.validate()
        ...
        with self.assertRaisesRegex(ValueError,
                                    expected_regex='SimpleGraphCycle is not valid : '
                                                   'Simple graph cycle with 2 nodes is not valid.'):
            sgc.validate()

        # Simple graph cycle with nodes that cannot be strictly ordered
        sgc = SimpleGraphCycle([FakeNodeWithEqMethodWrongSortable(0),
        ...
        sgc = SimpleGraphCycle([FakeNodeWithEqMethod(8),
        ...
        sgc = SimpleGraphCycle([FakeNodeWithEqLtMethods(8),
        ...
        self.assertEqual(sgc.nodes, (FakeNodeWithEqLtMethods(0),
                                     FakeNodeWithEqLtMethods(6),
                                     FakeNodeWithEqLtMethods(3),
                                     FakeNodeWithEqLtMethods(8)))

        sgc = SimpleGraphCycle([FakeNodeWithEqLtMethodsBis(8),
                                FakeNodeWithEqLtMethods(0),
                                FakeNodeWithEqLtMethods(6),
                                FakeNodeWithEqLtMethods(3)],
                               validate=False, ordered=False)
        sgc.order(raise_on_fail=False)
        self.assertFalse(sgc.ordered)
        self.assertEqual(sgc.nodes, (FakeNodeWithEqLtMethodsBis(8),
                                     FakeNodeWithEqLtMethods(0),
                                     FakeNodeWithEqLtMethods(6),
                                     FakeNodeWithEqLtMethods(3)))
        with self.assertRaisesRegex(ValueError,
                                    expected_regex='Could not order simple graph cycle as the nodes '
                                                   'are of different classes.'):
            sgc.order(raise_on_fail=True)

        sgc = SimpleGraphCycle([FakeNodeWithEqLtMethods(85)],
                               validate=False, ordered=False)
        self.assertFalse(sgc.ordered)
        sgc.order()
        self.assertTrue(sgc.ordered)
        ...
        sgc.order()
        self.assertTrue(sgc.ordered)
        self.assertEqual(sgc.nodes, tuple([FakeNodeWithEqLtMethods(1),
                                           FakeNodeWithEqLtMethods(4),
                                           FakeNodeWithEqLtMethods(3),
                                           FakeNodeWithEqLtMethods(6),
                                           FakeNodeWithEqLtMethods(2),
        ...
        # Test str method
        self.assertEqual(str(sgc), 'Simple cycle with nodes :\n'
                                   'FakeNode_1\n'
                                   'FakeNode_4\n'
                                   'FakeNode_3\n'
                                   'FakeNode_6\n'
    ...
    def test_multigraph_cycle(self):
        ...
        with self.assertRaisesRegex(ValueError, expected_regex='MultiGraphCycle is not valid : '
        ...
        mgc = MultiGraphCycle(nodes=[FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(2)],
                              edge_indices=[0, 0],
                              validate=False, ordered=False)
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(1)],
                              edge_indices=[0, 1, 0],
                              validate=False, ordered=False)
        with self.assertRaisesRegex(ValueError,
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0)],
                              edge_indices=[1, 1],
                              validate=False, ordered=False)
        with self.assertRaisesRegex(ValueError,
                                    expected_regex='MultiGraphCycle is not valid : '
        ...
        # Nodes not sortable
        mgc = MultiGraphCycle([FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(2)],
                              edge_indices=[0, 0, 0],
                              validate=False, ordered=False)
        self.assertFalse(mgc.ordered)
        self.assertEqual(mgc.nodes, (FakeNodeWithEqMethod(1), FakeNodeWithEqMethod(0), FakeNodeWithEqMethod(2)))
        mgc.validate(check_strict_ordering=False)
        with self.assertRaisesRegex(ValueError, expected_regex='MultiGraphCycle is not valid : '
                                                               'The nodes are not sortable.'):
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqMethodWrongSortable(0),
                               FakeNodeWithEqMethodWrongSortable(1),
                               FakeNodeWithEqMethodWrongSortable(2),
                               FakeNodeWithEqMethodWrongSortable(3)],
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqMethod(8),
                               FakeNodeWithEqMethod(0),
                               FakeNodeWithEqMethod(3),
                               FakeNodeWithEqMethod(6)],
                              edge_indices=[0, 0, 0, 0],
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqLtMethods(8),
                               FakeNodeWithEqLtMethods(0),
                               FakeNodeWithEqLtMethods(6),
                               FakeNodeWithEqLtMethods(3)],
        ...
        self.assertEqual(mgc.nodes, (FakeNodeWithEqLtMethods(0),
                                     FakeNodeWithEqLtMethods(6),
                                     FakeNodeWithEqLtMethods(3),
                                     FakeNodeWithEqLtMethods(8)))
        self.assertEqual(mgc.edge_indices, (5, 3, 7, 2))

        mgc = MultiGraphCycle([FakeNodeWithEqLtMethodsBis(8),
                               FakeNodeWithEqLtMethods(0),
                               FakeNodeWithEqLtMethods(6),
                               FakeNodeWithEqLtMethods(3)],
                              edge_indices=[2, 5, 3, 7],
        ...
        mgc.order(raise_on_fail=False)
        self.assertFalse(mgc.ordered)
        self.assertEqual(mgc.nodes, (FakeNodeWithEqLtMethodsBis(8),
                                     FakeNodeWithEqLtMethods(0),
                                     FakeNodeWithEqLtMethods(6),
                                     FakeNodeWithEqLtMethods(3)))
        ...
        mgc = MultiGraphCycle([FakeNodeWithEqLtMethods(85)],
                              edge_indices=[7],
                              validate=False, ordered=False)
        self.assertFalse(mgc.ordered)
        mgc.order()
        ...
        mgc.order()
        self.assertTrue(mgc.ordered)
        self.assertEqual(mgc.nodes, tuple([FakeNodeWithEqLtMethods(1),
                                           FakeNodeWithEqLtMethods(4),
                                           FakeNodeWithEqLtMethods(3),
                                           FakeNodeWithEqLtMethods(6),
                                           FakeNodeWithEqLtMethods(2),
        ...
        for inodes, iedges in [((0, 1, 2, 3), (3, 6, 9, 12)),
                               ((1, 2, 3, 0), (6, 9, 12, 3)),
                               ((2, 3, 0, 1), (9, 12, 3, 6)),
                               ((3, 0, 1, 2), (12, 3, 6, 9)),
                               ((3, 2, 1, 0), (9, 6, 3, 12)),
