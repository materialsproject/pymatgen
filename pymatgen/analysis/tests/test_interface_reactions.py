...
class InterfaceReactionTest(unittest.TestCase):
    ...
    def test_get_reaction(self):
        ...
        self.assertTrue(test1,
                        '_get_reaction: reaction not involving chempots'
                        ' species gets error!')

        test2 = str(self.ir[0]._get_reaction(0.5,
                                             normalize=1)) == '0.5 O2 + 0.5 Mn ' \
                                                              '-> 0.5 MnO2'
        self.assertTrue(test2,
                        '_get_reaction: reaction not involving chempots '
                        'species gets error!')
        ...
        test3 = str(self.ir[3]._get_reaction(
            0.666666)) == '2 Mn + 2 Li2O -> 4 Li + MnO2 + Mn' or \
                '2 Mn + 2 Li2O -> 4 Li + Mn + MnO2'
        self.assertTrue(test3,
                        '_get_reaction: reaction involving chempots species '
                        'gets error!')
    ...
    def test_convert(self):
        ...
        self.assertTrue(np.allclose(result, answer),
                        '_convert: conversion gets error! {0} expected, but'
                        ' gets {1}'.format(answer, result))
    ...
    def test_get_products(self):
        test1 = sorted(self.ir[0].get_products()) == sorted(
            ['MnO2', 'O2', 'Mn'])
        self.assertTrue(test1,
                        'get_products: decomposition products gets error for'
                        ' reaction not involving chempots species!')

        test2 = sorted(self.ir[3].get_products()) == sorted(
            ['Li', 'MnO2', 'Mn', 'Li2O'])
        self.assertTrue(test2,
                        'get_decomp: decomposition products gets error for'
                        ' reaction involving chempots species!')
    ...
    def test_minimum(self):
        ...
        for i, j in zip(self.ir, answer):
            self.assertTrue(np.allclose(i.minimum(), j),
                            'minimum: the system with {0} and {1} gets error!{2} '
                            'expected, but gets {3}'.format(
    ...
    def test_get_no_mixing_energy(self):
        ...
        self.assertTrue(
            'Please provide grand potential phase diagram for computing '
            'no_mixing_energy!' == str(context1.exception))
        answer = [
        ...
        for i, j in zip(result_info, answer):
            self.assertTrue(name_lst(i) == name_lst(j),
                            'get_no_mixing_energy: names get error, {0} expected'
                            ' but gets {1}'.format(name_lst(j), name_lst(i)))
            self.assertTrue(np.allclose(energy_lst(i), energy_lst(j)),
                            'get_no_mixing_energy: no_mixing energies get error,'
