...
class TestMoleculeStructureComparator(TestCase):
    def test_are_equal(self):
        ...
        mol1 = Molecule.from_file(os.path.join(test_dir, "t1.xyz.gz"))
        mol2 = Molecule.from_file(os.path.join(test_dir, "t2.xyz.gz"))
        mol3 = Molecule.from_file(os.path.join(test_dir, "t3.xyz.gz"))
        assert not msc1.are_equal(mol1, mol2)
        assert msc1.are_equal(mol2, mol3)
        thio1 = Molecule.from_file(os.path.join(test_dir, "thiophene1.xyz.gz"))
        thio2 = Molecule.from_file(os.path.join(test_dir, "thiophene2.xyz.gz"))
        # noinspection PyProtectedMember
        msc2 = MoleculeStructureComparator(priority_bonds=msc1._get_bonds(thio1))
        assert msc2.are_equal(thio1, thio2)
        hal1 = Molecule.from_file(os.path.join(test_dir, "molecule_with_halogen_bonds_1.xyz.gz"))
        hal2 = Molecule.from_file(os.path.join(test_dir, "molecule_with_halogen_bonds_2.xyz.gz"))
    ...
    def test_get_bonds(self):
        ...
        mol2 = Molecule.from_file(os.path.join(test_dir, "MgBH42.xyz.gz"))
        ...
        mol1 = Molecule.from_file(os.path.join(test_dir, "molecule_with_halogen_bonds_1.xyz.gz"))
        msc = MoleculeStructureComparator()
        # noinspection PyProtectedMember
        bonds = msc._get_bonds(mol1)
        assert bonds == [
    ...
    #     qcout_path = os.path.join(test_dir, "mol_1_3_bond.qcout.gz")
