...
class StructureFitterTest(unittest.TestCase):
    ...
    def test_init(self):
        ...
        fitter = StructureFitter(self.b, editor.modified_structure)
        self.assertTrue(fitter.mapping_op != None,
                        "No fit found for translation {}!".format(trans))
    ...
    def test_anonymized_fitting(self):
        ...
        self.assertEqual({el1.symbol: el2.symbol
                          for el1, el2 in fitter.el_mapping.items()},
                         {"O": "O", "Fe": "Fe", "Na": "Li", "P": "P"})
...
class SupportFunctionTest(unittest.TestCase):
    ...
    def test_shear_invariant(self):
        mat = np.array([[1.0000002761907518, 1.5947378062541873E-7, -2.2550605566218351E-7],
                        [1.5947378062541873E-7, 0.9999997238033649, -3.906039923173843E-7],
                        [-2.2550605566218351E-7, -3.906039922896287E-7, 1.0000000000061875]])
        self.assertAlmostEqual(0, shear_invariant(mat), 7)

    def test_sqrt_matrix(self):
        mat = np.array([[0.1, 0, 0], [0.2, 0.3, 0.1], [0.4, 0.7, 0.9]])
        expected_ans = np.array([[0.09813965, -0.13085287, 0.06542643],
                                 [-0.13085287, 0.41807729, -0.17084204],
                                 [0.06542643, -0.17084204, 1.24722442]])
