...
class BVAnalyzerTest(PymatgenTest):
    ...
    def test_get_valence(self):
        struct = Structure.from_file(os.path.join(PymatgenTest.TEST_FILES_DIR, "LiMn2O4.json.gz"))
        ...
        assert self.analyzer.get_valences(struct) == ans
        struct = Structure.from_file(os.path.join(PymatgenTest.TEST_FILES_DIR, "Li4Fe3Mn1(PO4)4.json.gz"))
        ans = [1] * 4 + [2] * 4 + [5] * 4 + [-2] * 16
        assert self.analyzer.get_valences(struct) == ans
