    ...
    def get_13_bonds(priority_bonds):
        ...
        all_2_and_13_bonds = {
            ...
            for b in itertools.chain(
                *[itertools.combinations(p, 2) for p in all_13_bond_atoms]
            )
        }
    ...
    def _get_bonds(self, mol):
        ...
        if self.ignore_ionic_bond:
            covalent_atoms = [
                ...
                if mol.species[i].symbol not in self.ionic_element_list
            ]
        else:
        ...
        if len(unavailable_elements) > 0:
            raise ValueError(
                "The covalent radius for element {} is not "
                "available".format(unavailable_elements)
            )
        bond_13 = self.get_13_bonds(self.priority_bonds)
        max_length = [
            (
                self.covalent_radius[mol.sites[p[0]].specie.symbol]
            ...
            * (
                ...
                + (
                    ...
                    if p in self.priority_bonds
                        ...
                        self.bond_length_cap if p not in bond_13 else self.bond_13_cap
                    )
                )
            )
            * (
