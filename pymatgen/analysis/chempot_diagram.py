...
with open(
    os.path.join(
        os.path.dirname(__file__), "..", "util", "plotly_chempot_layouts.json"
    )
) as f:
...
class ChemicalPotentialDiagram(MSONable):
    ...
    def __init__(
        ...
        """
        self.entries = list(
            sorted(entries, key=lambda e: e.composition.reduced_composition)
        )
        self.limits = limits
        ...
        self.elements = list(
            sorted(
                {els for e in self.entries for els in e.composition.elements}
            )
        )
        ...
        self._entry_dict = {
            e.composition.reduced_formula: e for e in self._min_entries
        }
    ...
    def get_plot(
        ...
        Elemental axes can be specified; if none provided, will automatically default to first 2-3
        elements within the "elements" attribute.
        ...
        if len(elems) == 2 and self.dim == 2:
            ...
            entries = [
                ...
                if set(e.composition.elements).issubset(elems)
            ]
    ...
    def _get_domains(self):
        ...
        for intersection, facet in zip(
            hs_int.intersections, hs_int.dual_facets
        ):
            for v in facet:
                if v < len(entries):
    ...
    def _get_border_hyperplanes(self) -> np.ndarray:
        ...
        border_hyperplanes = np.array(
            ([[0] * (self.dim + 1)] * (2 * self.dim))
        )
    ...
    def _get_hyperplanes_and_entries(self) -> Tuple[np.ndarray, List[PDEntry]]:
        ...
        inds = np.where(form_e < -PhaseDiagram.formation_energy_tol)[
            0
        ].tolist()
        inds.extend(
            [self._min_entries.index(el) for el in self.el_refs.values()]
        )
    ...
    def _get_2d_plot(
        self, elements: List[Element], label_stable: Optional[bool]
    ) -> Figure:
    ...
    def _get_3d_plot(
        ...
        for formula, pts in domains.items():
            ...
            simplexes, ann_loc = self._get_3d_domain_simplexes_and_ann_loc(
                pts_3d
            )
        ...
        if draw_formula_lines:
            data.extend(
                self._get_3d_formula_lines(draw_domains, formula_colors)
            )
        if draw_formula_meshes:
            data.extend(
                self._get_3d_formula_meshes(draw_domains, formula_colors)
            )
    ...
    @staticmethod
    def _get_2d_domain_lines(domains, elements) -> List[Scatter]:
    ...
    @staticmethod
    def _get_3d_domain_lines(domains) -> List[Scatter3d]:
    ...
    @staticmethod
    def _get_3d_formula_meshes(draw_domains, formula_colors) -> List[Mesh3d]:
    ...
    @staticmethod
    def _get_3d_formula_lines(draw_domains, formula_colors) -> List[Scatter3d]:
    ...
    def _get_annotation(ann_loc, formula) -> Dict[str, Union[str, float]]:
        ...
        annotation.update(
            {"x": ann_loc[0], "y": ann_loc[1], "text": formula,}
        )
        if len(ann_loc) == 3:
    ...
    @staticmethod
    def _get_axis_layout_dict(elements) -> Dict[str, str]:
    ...
    @property
    def el_refs(self) -> np.ndarray:
        """ Returns elemental reference entries"""
    ...
    def __repr__(self):
        return (
            f"ChemicalPotentialDiagram for {self.chemical_system} with {len(self.entries)} "
            f"entries"
        )


def simple_pca(
    data: np.ndarray, k: int = 2
...
def get_2d_orthonormal_vector(line_pts):
    """
