...
class PourbaixAnalyzer(object):
    ...
    def get_chempot_range_map(self):
        ...
        if len(all_chempots) <= len(self._keys):
            ...
            mufacets_pyhull = ConvexHull(tol * np.array(all_chempots)).vertices
            mufacets = mufacets_pyhull
        ...
        for vertex in edge_vertices:
            ...
            if len(corner_pts_add) > 1:
                ...
                if len(points_on_border[vertex]) > 2:
                    raise StandardError("Undefined region. More than 2 points lie on same border!")
                line = []
