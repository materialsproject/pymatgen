...
class EwaldSumMatrix:
    """
    Calculates the electrostatic energy of a periodic array of charges using the Ewald technique.
    Returns the energies as a matrix of pairwise interactions (energy due to site i is
    obtained by summing the cells in row i and column i, remembering not to double count cell i,i)
    Doesn't do forces
    References : http://www.ee.duke.edu/~ayt/ewaldpaper/ewaldpaper.html
    ...
    def _calc_recip(self):
        ...
        self._recip = erecip * prefactor * EwaldSumMatrix.CONV_FACT 
    ...
    def _calc_real_and_point(self):
         ...
         * -(eta/pi)**(1/2) * sum_{i=1}^{N} q_i**2
         * 
         * a compensating background is added (i.e. a G=0 term) for all sites
         * (if the cell is charge neutral these sum to 0)
         *  
         * @return
         */
        """
    ...
    def __str__(self):
        output = ["Real = " + str(sum(sum(self.real_space_energy)))]
        output.append("Reciprocal = " + str(sum(sum(self.reciprocal_space_energy))))
        output.append("Point = " + str(sum(self.point_energy)))
        output.append("Total = " + str(sum(sum(self.total_energy))))
        return "\n".join(output)
