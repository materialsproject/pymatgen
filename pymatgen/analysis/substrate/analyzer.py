...
class ZurSuperLatticeGenerator(object):
    ...
    def rel_angle(self,vec_set1, vec_set2):
        ...
        """
        return self.vec_angle(vec_set2[0],vec_set2[1])/self.vec_angle(vec_set1[0],vec_set1[1])-1

    def is_same_vectors(self, vec_set1, vec_set2):
        """
            Determine if two sets of vectors are the same within length and angle tolerances specified in the criteria dictionary
        """
        return np.absolute(self.rel_strain(vec_set1[0],vec_set2[0])) < self.criteria["max_length_tol"] and\
            np.absolute(self.rel_strain(vec_set1[1],vec_set2[1])) < self.criteria["max_length_tol"] and\
            np.absolute(self.rel_angle(vec_set1,vec_set2)) < self.criteria["max_angle_tol"]
    ...
    def generate_sl_transformation(self, area_multiple):
        """
            ...
            Args:
                area_multiple: integer multiple of unit cell area for super lattice area
    ...
    def generate_sl_transformations(self, film_area, substrate_area):
        """
            Generates transformation sets for film/substrate pair given the area of the unit cell area for the film and substrate. The transformation sets map the film and substrate unit cells to super lattices with a maximum area given in criteria["max_area"]
            ...
            Returns:
                transformation_sets: a set of transformation_sets defined as:
                    1.) the (i,j) pair corresponding to the integer multiple of the film area (i) and substrate area (j) that makes the two equa within tolerance
                    2.) the transformation matricies for the film to create a super lattice of area i*film area
                    3.) the tranformation matricies for the substrate to create a super lattice of area j*film area

        """

        for i in range(1, int(self.criteria["max_area"]/film_area)):
            for j in range (1,int(self.criteria["max_area"]/substrate_area)):
                if gcd(i,j) == 1 and np.absolute(film_area/substrate_area - float(j)/i) < self.criteria["max_area_ratio_tol"]:
                    yield [(i,j),self.generate_sl_transformation(i),self.generate_sl_transformation(j)]

    def check_transformations(self, transformation_sets, film_vectors, substrate_vectors):
        ...
        for t in transformation_sets:
            ...
            for f in t[1]:
                films.append(self.reduce_vectors(*np.squeeze(np.asarray(f*film_vectors))))
            for s in t[2]:
                substrates.append(self.reduce_vectors(*np.squeeze(np.asarray(s*substrate_vectors))))
    ...
    def generate_slabs(self,film_millers,substrate_millers):
        ...
        for f in film_millers:
            ...
            film_vectors = self.reduce_vectors(film_slab.lattice_vectors()[0],film_slab.lattice_vectors()[1])
            ...
            for s in substrate_millers:
                ...
                substrate_vectors = self.reduce_vectors(substrate_slab.lattice_vectors()[0],substrate_slab.lattice_vectors()[1])
                substrate_area = self.area(*substrate_vectors)

                yield [film_area,substrate_area,film_vectors,substrate_vectors,f]
    ...
    def generate(self,film_millers = None, substrate_millers = None):
        ...
        if film_millers == None:
            film_millers = sorted(get_indices(self.film,self.criteria["film_max_miller_index"]))

        # Generate miller indicies if none specified for substrate
        if substrate_millers == None:
            substrate_millers = sorted(get_indices(self.substrate,self.criteria["sub_max_miller_index"]))


        # Check each miller index combination
        for slab_combination in self.generate_slabs(film_millers,substrate_millers):
            # Generate all super lattice comnbinations for a given set of miller indicies
            transformations = self.generate_sl_transformations(slab_combination[0],slab_combination[1])
            # Check each super-lattice pair to see if they match
            for match in self.check_transformations(transformations,slab_combination[2],slab_combination[3]):
