...
"""
This module provides classes to perform fitting of molecule with arbitrary atom orders.
"""
...
class AbstractMolAtomMapper(MSONable):
    '''
    Abstract molecular atom order mapping class. A mapping will be able to find the uniform 
    atom order of two molecules that can pair the geometrically equivalent atoms.
    '''
    ...
    def uniform_labels(self, mol1, mol2):
        ...
        Returns:
            (list1, list2) if uniform atom order is found. list1 and list2 are for mol1 and mol2, respectively. Their length equal
            to the number of atoms. They represents the uniform atom order of the two molecules. The
            value of each element is the original atom index in mol1 or mol2 of the current atom in
            uniform atom order. 
...
class InchiMolAtomMapper(AbstractMolAtomMapper):
    ...
    def _inchi_labels(self, mol):
        ...
        Returns:
            The label mappings. List of tuple of canonical label, original label
    ...
    def _virtual_molecule(self, mol, ilabels, eq_atoms, farthest_group_idx):
        '''
        Create a virtual molecule by unique atoms, the centriods of the equivalent 
        atoms, special atoms
        ...
        Args:
            ...
            farthest_group_idx:
                The equivalent atom group index in which there is the farthest atom 
                to the centroid
        ...
        if vmol.NumAtoms() < 3:
            ...
            for i in symm:
                orig_idx = ilabels[i-1]
                oa1 = mol.GetAtom(orig_idx)
                current_distance = math.sqrt((c1x - float(oa1.x()))**2 + (c1y - float(oa1.y()))**2\
                                               + (c1z - float(oa1.z()))**2)
            ...
            if vmol.NumAtoms() < 3:
                ...
                # all other atoms are hydrogen, a hydrogen can't connect two atoms
                # so they must be bonded to each other
                if vmol.NumAtoms() !=2:
                ...
                for symm in eq_atoms:
                    for i in symm:
                        orig_idx = ilabels[i-1]
                        oa1 = mol.GetAtom(orig_idx)
                        if oa1.IsConnected(mol.GetAtom(orig_last_idx)) and (orig_idx != orig_last_idx):
    ...
    def _largest_radius_group_idx(self, mol, ilabels, eq_atoms): 
        '''
        Find the equivalent atom group index which the farthest atom to centroid is
        located in
    ...
    def _align_heavy_atoms(self, mol1, mol2, ilabel1, ilabel2, eq_atoms):
        '''
        Align the label of topologically identical atoms of second molecule towards first molecule
        ...
        farthest_group_idx, farthest_distance = self._largest_radius_group_idx(mol1, ilabel1, eq_atoms)
        farthest_group_idx2, farthest_distance2 = self._largest_radius_group_idx(mol2, ilabel2, eq_atoms)
        if farthest_distance2 > farthest_distance:
            farthest_distance = farthest_distance2
            farthest_group_idx = farthest_group_idx2
        
        vmol1 = self._virtual_molecule(mol1, ilabel1, eq_atoms, farthest_group_idx)
        vmol2 = self._virtual_molecule(mol2, ilabel2, eq_atoms, farthest_group_idx)
        
        num_virtual_atoms = vmol1.NumAtoms()
        num_heavy_atoms = len(ilabel1)
        ...
        for i in ilabel2: # add all heavy atoms
            ...
            oa2 = mol2.GetAtom(i)
            a2 = vmol2.NewAtom()
            a2.SetAtomicNum(1)# align using the virtual atoms, these atoms are not
        ...
        for i in range(num_virtual_atoms + 1, num_virtual_atoms + num_heavy_atoms +1):
            oa2 = vmol2.GetAtom(i)
            a2 = aligned_mol2.NewAtom()
            a2.SetAtomicNum(oa2.GetAtomicNum())
            a2.SetVector(oa2.GetVector())
        
        canon_label2 = range(1, num_heavy_atoms+1)
        for symm in eq_atoms:
            for i in symm:
                canon_label2[i-1] = -1
        for symm in eq_atoms:
        ...
        canon_inchi_orig_map2 = [(canon, inchi, orig) for canon, inchi ,orig in \
                                 zip(canon_label2, range(1, num_heavy_atoms +1), ilabel2)]
    ...
    def _align_hydrogen_atoms(self, mol1, mol2, heavy_indices1, heavy_indices2):
        '''
        Align the label of topologically identical atoms of second molecule towards first molecule
        ...
        hydrogen_canon_orig_map2 = [(canon, orig) for canon, orig in zip(hydrogen_label2, hydrogen_orig_idx2)]
    ...
    def uniform_labels(self, mol1, mol2):
        ...
        heavy_atom_indices2 = self._align_heavy_atoms(obmol_orig_1, obmol_orig_2, ilabel1, ilabel2, iequal_atom1)
        clabel1, clabel2 = self._align_hydrogen_atoms(obmol_orig_1, obmol_orig_2, ilabel1, heavy_atom_indices2)
...
class MoleculeMatcher(MSONable):
    ...
    def __init__(self, tolerance=0.01, mapper=InchiMolAtomMapper()):
        ...
        Args:
            ...
            mapper:
                MolAtomMapper object that is able to map the atoms of two molecule to
                uniform order
        '''
    ...
    def _calc_rms(self, mol1, mol2, clabel1, clabel2):
        ...
        Args:
            ...
            mol2:
                The second molecule. OpenBabel OBMol or pymatgen Molecule object
            clabel1:
                The atom indices that can reorder the first molecule to uniform atom order
            clabel1:
                The atom indices that can reorder the second molecule to uniform atom order
    ...
    def group_molecules(self, mol_list):
        ...
        '''
        mol_hash = [(i, self._mapper.get_molecule_hash(m)) for i, m in \
                    enumerate(mol_list)]
        mol_hash.sort(key=lambda x: x[1])
        
        #Use molecular hash to pre-group molecules.
        raw_groups = tuple([tuple([m[0] for m in g]) for k, g in \
                            itertools.groupby(mol_hash, key=lambda x: x[1])])
        
        group_indices = []
        for rg in raw_groups:
            mol_eq_test = [(p[0], p[1], self.fit(mol_list[p[0]], mol_list[p[1]])) \
                           for p in itertools.combinations(sorted(rg), 2)]
            mol_eq = set([(p[0], p[1]) for p in mol_eq_test if p[2]==True])
            ...
            while len(not_alone_mols) > 0:
                ...
                while len(not_alone_mols) > 0:
                    candidate_pairs = set([tuple(sorted(p)) for p in \
                                           itertools.product(current_group, not_alone_mols)])
    ...
    @staticmethod
    def from_dict(d):
        return MoleculeMatcher(tolerance=d["tolerance"], mapper=AbstractMolAtomMapper.from_dict(d["mapper"]))
    
    
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Check whether two molecules have the same \
                                     structure and geometry or group molecule for more than 3 \
                                     molecules")
    parser.add_argument("molecules", type=argparse.FileType(mode='r'), nargs='+', metavar="FILENAME",
                        help="The filename of the two molecules to be compared")
    parser.add_argument("-f", "--format", dest="format", default="xyz",
                        help="format of the molecule files OpenBabel Abbreviations")
    parser.add_argument("-t", "--tolerance", dest="tolerance", default="0.01", type=float,
                        help="tolerance of RMDS comparison, in Angstrom")
    parser.add_argument("-g", "--group_prefix", dest="group_prefix", default="mol_",
