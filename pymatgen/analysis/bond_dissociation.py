...
class BondDissociationEnergies(MSONable):
    ...
    def __init__(
        ...
        for key in required_keys:
            ...
            for entry in self.filtered_entries:
                if key not in entry:
                    raise RuntimeError(
                        key + " must be present in all fragment entries! Exiting..."
                    )
    ...
    def search_fragment_entries(self, frag):
        ...
        final_entries = []
        for entry in self.filtered_entries:
            if frag.isomorphic_to(entry["initial_molgraph"]) and frag.isomorphic_to(
    ...
    def filter_fragment_entries(self, fragment_entries):
        ...
        for entry in fragment_entries:
            ...
            if entry["initial_molgraph"].isomorphic_to(entry["final_molgraph"]):
                ...
                if nx.is_connected(
                    initial_graph.to_undirected()
                ) and not nx.is_connected(final_graph.to_undirected()):
            ...
            for ii, filtered_entry in enumerate(self.filtered_entries):
                if filtered_entry["formula_pretty"] == entry["formula_pretty"]:
                    if (
                        filtered_entry["initial_molgraph"].isomorphic_to(
                            entry["initial_molgraph"]
                        )
                        and filtered_entry["final_molgraph"].isomorphic_to(
                            entry["final_molgraph"]
                        )
                        and filtered_entry["initial_molecule"]["charge"]
    ...
    def build_new_entry(self, frags, bonds):
        ...
        if len(frags) == 2:
            new_entry = [
                self.molecule_entry["final_energy"]
