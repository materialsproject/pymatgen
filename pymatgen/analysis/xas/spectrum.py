...
class XANES(Spectrum):
    ...
    Args:
        ...
        absorption_element (Element): Element associated with the XANES
        edge (str): Absorption edge associated with the XANES
    ...
    def __init__(self, x, y, structure, absorption_element, edge):
        super(XANES, self).__init__(x, y, structure, absorption_element, edge)
...
class EXAFS(Spectrum):
    ...
    Args:
        x: A sequence of x-ray energies in eV
        y: A sequence of mu(E)
        structure (Structure): Structure associated with the EXAFS
        absorption_element (Element): Element associated with the EXAFS
        edge (str): Absorption edge associated with the EXAFS
    ...
    def __init__(self, x, y, structure, absorption_element, edge):
        super(EXAFS, self).__init__(x, y, structure, absorption_element, edge)
        self.structure = structure
        self.absorption_element = absorption_element
        self.edge = edge
        self.e0 = self.x[np.argmax(np.gradient(self.y) / np.gradient(self.x))]
    ...
    def __str__(self):
        return "%s %s Edge for %s: %s" % (
            self.absorption_element, self.edge,
            self.structure.composition.reduced_formula,
            super(EXAFS, self).__str__()
        )


def stitch_xanes_exafs(xanes, exafs, num_samples=500):
    """
...
def stitch_l23(l2_xanes, l3_xanes, num_samples=200):
    ...
    l3_f = interp1d(
        l2_xanes.x, l2_xanes.y, bounds_error=False, fill_value=l3_xanes.y[-1])
