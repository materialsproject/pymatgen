...
from pymatgen.command_line.qhull_caller import *
...
class QhullCallerTest(unittest.TestCase):
    ...
    def test_qconvex(self):
        ...
        facets = qconvex(data)
        self.assertEqual(facets, [[0, 2], [1, 0], [2, 3], [3, 1]], "Qconvex gave wrong answers")
    
    def test_qvertex(self):
        data = [[-0.5,0,-0.5],[-0.5,0.5,0],[0.5,-0.5,1],[0.5,0.5,2],[0.5,1,2]]
        facets = qvertex(data)
        print "Qvertex output : " + str(facets)
        self.assertEqual(facets, [[8.0, 3.75, -3.75], [0.5, 0.75, 0.75]], "Qvertex gave wrong answers")
