...
from pymatgen.io.pwmat.inputs import AtomConfig, ACstrExtractor, LineLocator
...
class Movement(MSONable):
    def __init__(
        self,
        filename: PathLike,
        ionic_step_skip: Union[int, None] = None,
        ionic_step_offset: Union[int, None] = None):
        '''
        Description:
    ...
    def _get_chunkinfo(self):
        ...
        '''
        chunksizes:List[int] = []
        row_idxs:List[int] = LineLocator.locate_all_lines(
            self.filename, 
            self.split_mark)
        ...
        with zopen(self.filename, "rt") as mvt:
            ...
            for ii in range(self.nionic_steps):
                ...
                try:
                    ...
                    print("Ionic step #{0} : Energy deposition is turn down.".format(ii))
                try:
                    tmp_step.update({"virial": ACstrExtractor(tmp_chunk).get_virial().reshape(3, 3)}) 
                except:
                    print("Ionic step #{0} : No virial infomation.".format(ii))
