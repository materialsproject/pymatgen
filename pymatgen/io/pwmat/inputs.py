...
class ACExtractor(ACExtractorBase):
    ...
    def get_lattice(self):
        ...
        content:str = "LATTICE"    
        idx_row:int = LineLocator.locate_all_lines(
...
class AtomConfig(MSONable):    
    def __init__(
        self, 
        structure:Structure,
        sort_structure:Optional[bool]=False):
        """
        Args:
        ...
        true_names = ["{0}{1}".format(tmp_key, tmp_value) for (tmp_key, tmp_value) in elements_counter.items()]
    ...
    def get_str(self):
        ...
        lines:List[str] = []
        lines.append("\t{0} atoms\n".format(self.structure.num_sites))
        lines.append("Lattice vector\n")
        for ii in range(3):
            lines.append("{0:>15f}{1:>15f}{2:>15f}\n".format(latt.matrix[ii][0], latt.matrix[ii][1], latt.matrix[ii][2]))
        lines.append("Position, move_x, move_y, move_z\n")
        for ii in range(self.structure.num_sites):
            lines.append("{0:>4d}{1:>15f}{2:>15f}{3:>15f}{4:>4d}{5:>4d}{6:>4d}\n".format(
        ...
        if "magmom" in self.structure.sites[0].properties:
            ...
            for ii, tmp_site in enumerate(self.structure.sites):
                lines.append("{0:>4d}{1:>15f}\n".format(
                    int( tmp_site.specie.Z ),
                    tmp_site.properties["magmom"]
                ))
        return "".join(lines)
