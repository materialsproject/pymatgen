...
class GenKpt(MSONable):
    ...
    def get_str(self):
        def calc_distance(hsp1:str, hsp2:str):
            hsp1_coord:np.array = np.dot( np.array(self._kpath["kpoints"][hsp1]).reshape(1, 3), self._reciprocal_lattice )
            hsp2_coord:np.array = np.dot( np.array(self._kpath["kpoints"][hsp2]).reshape(1, 3), self._reciprocal_lattice )
        ...
        genkpt_str:str = f"K-path generated by pymatgen. density={self._density}, fractional coordinates in reciprocal lattice.\n"
        for ii in range(len(flatten_paths)-1):
            genkpt_str += f"{np.ceil( calc_distance(flatten_paths[ii], flatten_paths[ii+1]) / self._density )}\n"
            genkpt_str += f"    {self._kpath['kpoints'][flatten_paths[ii]][0]:>12.6f}\t{self._kpath['kpoints'][flatten_paths[ii]][1]:>12.6f}\t{self._kpath['kpoints'][flatten_paths[ii]][2]:>12.6f}\t{flatten_paths[ii]}\n"
            genkpt_str += f"    {self._kpath['kpoints'][flatten_paths[ii+1]][0]:>12.6f}\t{self._kpath['kpoints'][flatten_paths[ii+1]][1]:>12.6f}\t{self._kpath['kpoints'][flatten_paths[ii+1]][2]:>12.6f}\t{flatten_paths[ii+1]}\n"
        print(genkpt_str)
