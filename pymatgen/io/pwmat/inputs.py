...
class GenKpt(MSONable):
    ...
    def __init__(
        self, reciprocal_lattice: np.array, kpoints: dict[str, np.array], path: list[list[str]], density: float = 0.03
    ):
    ...
    @staticmethod
    def from_structure(structure: Structure, dim: int, density: float = 0.03):
        """
        Args:
            strutcure (Structure):
    ...
    def get_str(self):
        ...
        genkpt_str: str = (
            f"K-path generated by pymatgen. density={self._density}, fractional coordinates in reciprocal lattice.\n"
        )
