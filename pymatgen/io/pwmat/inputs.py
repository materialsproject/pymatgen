    ...
    def get_str(self):
        def calc_distance(hsp1:str, hsp2:str):
            hsp1_coord:np.array = np.dot( 
                np.array(self._kpath["kpoints"][hsp1]).reshape(1, 3), 
                self._reciprocal_lattice)
            hsp2_coord:np.array = np.dot(
                np.array(self._kpath["kpoints"][hsp2]).reshape(1, 3), 
                self._reciprocal_lattice)
        ...
        genkpt_str:str = f"Generated by pymatgen. density={self._density}, "
