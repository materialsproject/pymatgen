...
class GenKpt(MSONable):
    ...
    def get_str(self):
        ...
        genkpt_str: str = f"Generated by pymatgen. density={self.density/(2*np.pi)}, "
        ...
        for ii in range(len(flatten_paths) - 1):
            if [flatten_paths[ii], flatten_paths[ii + 1]] not in discontinue_pairs:
                genkpt_str += f"{np.ceil(calc_distance(flatten_paths[ii], flatten_paths[ii+1])/self.density)}\n"
                genkpt_str += f"    {self.kpath['kpoints'][flatten_paths[ii]][0]:>12.6f}\t"
                genkpt_str += f"{self.kpath['kpoints'][flatten_paths[ii]][1]:>12.6f}\t"
                genkpt_str += f"{self.kpath['kpoints'][flatten_paths[ii]][2]:>12.6f}\t"
                genkpt_str += f"{flatten_paths[ii]}\n"
                genkpt_str += f"    {self.kpath['kpoints'][flatten_paths[ii+1]][0]:>12.6f}\t"
                genkpt_str += f"{self.kpath['kpoints'][flatten_paths[ii+1]][1]:>12.6f}\t"
                genkpt_str += f"{self.kpath['kpoints'][flatten_paths[ii+1]][2]:>12.6f}\t"
