...
"""
This module implements input and output processing from Fiesta (http://perso.neel.cnrs.fr/xavier.blase/fiesta/index.html)

and

-Nwchem2Fiesta class: allows to run NWCHEM2FIESTA to create the input files needed for a Fiesta run
-Fiesta_run: allows to run gw_fiesta and bse_fiesta
...
class Nwchem2Fiesta(MSONable):
    """
    To run NWCHEM2FIESTA inside python after a nwchem run:
	if nwchem.nw is the input, nwchem.out the output, and structure.movecs the "movecs" file, the syntax to run NWCHEM2FIESTA is:

	NWCHEM2FIESTA nwchem.nw  nwchem.nwout  structure.movecs > log_n2f

    """

    def __init__(self, folder, filename="nwchem", log_file="log_n2f"):
        """
        folder: where are stored the nwchem files
...
class FiestaInput(MSONable):
    """
    Input File for Fiesta called "cell.in" by default (mandatory in Fiesta so far)
    """
...
class BSEOutput(object):
    """
    A bse output file parser. The first draft
