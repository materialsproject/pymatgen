...
class Cp2kInputSet(Cp2kInput):
    ...
    def create_subsys(self, structure: Union[Structure, Molecule]):
        ...
        basis_and_potential = get_basis_and_potential(
            [s for s in structure.symbol_set],
            self.basis_and_potential)
        ...
        for k in unique_kinds.keys():
            ...
            _ox = self.structure.site_properties['oxi_state'][unique_kinds[k][0]] \
                if 'oxi_state' in self.structure.site_properties else 0
            _sp = self.structure.site_properties['spin'][unique_kinds[k][0]] \
                if 'spin' in self.structure.site_properties else 0

            bs = BrokenSymmetry.from_el(
                kind,
                _ox,
                _sp
            ) if _ox else None
