...
class LobsterinTest(unittest.TestCase):
    def setUp(self):
        ...
        #check for value errors if lobsterin has more than one entry for the same keyword!

    def test_from_file(self):
        print(self.Lobsterinfromfile)
    ...
    def test_initialize_from_dict(self):
        lobsterin1=Lobsterin({'cohpstartenergy': -15.0, 'cohpendenergy': 5.0, 'basisset': 'pbeVaspFit2015', 'gaussiansmearingwidth': 0.1,
    ...
    def test_standard_settings(self):
        #test if all settings are correct if poscar, potcar, incar etc are provided
        #check again! something does not work here
        for option in ['standard','standard_from_projection','standard_with_fatband','onlyprojection','onlydos','onlycohp','onlycoop','onlycohpcoop']:
            lobsterin1=Lobsterin.standard_calculations_from_vasp_files(os.path.join(test_dir_doscar, "POSCAR.Fe3O4"),os.path.join(test_dir_doscar, "INCAR.lobster"),os.path.join(test_dir_doscar, "POTCAR.Fe3O4"),option=option)
            ...
            if option in ['standard','standard_with_fatband','onlyprojection','onlycohp','onlycoop','onlycohpcoop']:
    ...
    def test_diff(self):
        ...
        self.assertDictEqual(self.Lobsterinfromfile.diff(self.Lobsterinfromfile3)["Same"],self.Lobsterinfromfile3.diff(self.Lobsterinfromfile)["Same"])
        self.assertListEqual(list(self.Lobsterinfromfile.diff(self.Lobsterinfromfile3)["Different"].keys()),list(self.Lobsterinfromfile3.diff(self.Lobsterinfromfile)["Different"].keys()))
        self.assertEqual(self.Lobsterinfromfile.diff(self.Lobsterinfromfile3)["Different"]["SKIPCOHP"]["lobsterin1"],self.Lobsterinfromfile3.diff(self.Lobsterinfromfile)["Different"]["SKIPCOHP"]["lobsterin2"])

    def test_get_basis(self):
        lobsterin1=Lobsterin({})
        self.assertListEqual(lobsterin1._get_basis(Structure.from_file(os.path.join(test_dir_doscar, "Fe3O4.cif")),POTCAR=os.path.join(test_dir_doscar, "POTCAR.Fe3O4")),['Fe 3d 4p 4s ', 'O 2p 2s '])


    def test_write_lobsterin(self):
        #write some dummy files and check
        pass

    def test_write_INCAR(self):
        #write dummy file and check
        pass
        #make sure incar is correctly printed

    def test_write_KPOINTS(self):
        #write dummy file and check
        pass
        #make sure kpoints with and without kpath are correctly printed
