...
class QCOutput(MSONable):
    ...
    def __init__(self, filename):
        ...
        if read_pattern(self.text, {"key": r"SCF failed to converge"}, terminate_on_match=True).get(
            "key"
        ) == [[]]:
        ...
        if self.data["solvent_method"] == "PCM":
            ...
            self.data["solvent_data"] = {}
            temp_solvent = read_pattern(self.text, {"key": r"\s[Ss]olvent:? ([a-zA-Z]+)"}).get(
                "key"
            )
        ...
        if self.data.get("using_dft_d3", []):
            temp_d3 = read_pattern(
                self.text, {"key": r"\-D3 energy without 3body term =\s*([\d\.\-]+) hartrees"},
            ).get("key")
        ...
        if self.data.get("unrestricted", []):
            correct_s2 = (
                0.5 * (self.data["multiplicity"] - 1) * (0.5 * (self.data["multiplicity"] - 1) + 1)
            )
        ...
        self.data["transition_state"] = read_pattern(
            self.text, {"key": r"(?i)\s*job(?:_)*type\s*(?:=)*\s*ts"}
        ).get("key")
        if self.data.get("transition_state", list()):
            self._read_optimization_data()

        # Check if the calculation contains a constraint in an $opt section.
        self.data["opt_constraint"] = read_pattern(self.text, {"key": r"\$opt\s+CONSTRAINT"}).get(
            "key"
        )
    ...
    def _read_SCF(self):
        ...
        if self.data.get("using_GEN_SCFMAN", []):
            ...
            if "SCF_failed_to_converge" in self.data.get("errors"):
                footer_pattern = (
                    r"^\s*\d+\s*[\d\-\.]+\s+[\d\-\.]+E[\d\-\.]+\s+Convergence\s+failure\n"
                )
            else:
                footer_pattern = r"^\s*\-+\n"
    ...
    def _read_charges(self):
        ...
        if esp_or_resp is not None:
            header_pattern = (
                r"Merz-Kollman (R?ESP) Net Atomic Charges\s+Atom\s+Charge \(a\.u\.\)\s+\-+"
            )
            table_pattern = r"\s+\d+\s\w+\s+([\d\-\.]+)"
            footer_pattern = r"\s\s\-+\s+Sum of atomic charges"
    ...
    def _detect_general_warnings(self):
        ...
        if read_pattern(self.text, {"key": r"Intel MKL ERROR"}, terminate_on_match=True).get(
            "key"
        ) == [[]]:
        ...
        if read_pattern(
            self.text, {"key": r"Linear dependence detected in AO basis"}, terminate_on_match=True,
        ).get("key") == [[]]:
        ...
        if read_pattern(
            self.text, {"key": r"\*\*WARNING\*\* Magnitude of eigenvalue"}, terminate_on_match=True,
        ).get("key") == [[]]:
    ...
    def _read_geometries(self):
        ...
        geoms = []
        header_pattern = (
            r"\s+Optimization\sCycle:\s+\d+\s+Coordinates \(Angstroms\)\s+ATOM\s+X\s+Y\s+Z"
        )
        ...
        if parsed_optimized_geometries == [] or None:
            ...
            self.data["optimized_geometries"] = [
                process_parsed_coords(i) for i in parsed_optimized_geometries
            ]
            if self.data.get("charge") is not None:
    ...
    def _read_gradients(self):
        ...
        if self.data["solvent_method"] is not None:
            header_pattern = (
                r"total gradient after adding PCM contribution --\s+-+\s+Atom\s+X\s+Y\s+Z\s+-+"
            )
        ...
        if read_pattern(self.text, {"key": r"Gradient of CDS energy"}, terminate_on_match=True).get(
            "key"
        ) == [[]]:
    ...
    def _read_frequency_data(self):
        ...
        if temp_dict.get("frequencies") is None:
            ...
            if not raman:
                header_pattern = (
                    r"\s*Raman Active:\s+[YESNO]+\s+(?:[YESNO]+\s+)*X\s+Y\s+Z\s+(?:X\s+Y\s+Z\s+)*"
                )
            else:
            ...
            for ii, triple_FMV in enumerate(temp_freq_mode_vecs):
                for jj, line in enumerate(triple_FMV):
                    for kk, entry in enumerate(line):
                        if entry != "None":
                            freq_mode_vecs[int(ii * 3 + math.floor(kk / 3)), jj, kk % 3] = float(
                                entry
                            )
    ...
    def _check_completion_errors(self):
        ...
        if read_pattern(
        ...
        elif read_pattern(self.text, {"key": r"method not available"}, terminate_on_match=True).get(
            "key"
        ) == [[]]:
        ...
        elif read_pattern(self.text, {"key": r"Welcome to Q-Chem"}, terminate_on_match=True).get(
            "key"
        ) != [[]]:
            ...
            self.text, {"key": r"Unable to validate license"}, terminate_on_match=True
        ).get("key") == [[]]:
            self.data["errors"] += ["licensing_error"]
        elif read_pattern(
            self.text,
