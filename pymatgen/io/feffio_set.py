...
class AbstractFeffInputSet(object):
    ...
    The idea is that using a FeffInputSet, a complete set of input files (feffPOT,feffXANES, feffEXAFS, ATOMS, feff.inp)
    ...
    def get_header(self, structure, cif_file):
        '''
        Returns header to be used in feff.inp file from a structure and cif_file
        cif_file is name of cif_file used to creat structure
        '''
        return
    ...
    def get_all_feff_input(self, structure, calc_type, cif_file, central_atom):
        ...
        '''
        feff= {'HEADER':self.get_header(structure, cif_file), 'PARAMETERS':self.get_fefftags(calc_type),
                    'POTENTIALS': self.get_feffPot(structure, central_atom), 'ATOMS': self.get_feffAtoms(structure, central_atom)}

        return feff

    def write_input(self, structure, calc_type, cif_file, output_dir, central_atom, make_dir_if_not_present=True):
        """
        ...
        feff=self.get_all_feff_input(structure, calc_type, cif_file, central_atom)
        feff_input=str(feff["HEADER"])+"\n\n"+str(feff["PARAMETERS"])+"\n\n"+str(feff["POTENTIALS"])+"\n\n"+str(feff["ATOMS"])     
        for k, v in self.get_all_feff_input(structure, calc_type, cif_file, central_atom).items():
...
class FeffInputSet(AbstractFeffInputSet):
    ...
    def __init__(self, name):
        ...
        self._config.readfp(open(os.path.join(module_dir, "FeffInputSets.cfg")))
