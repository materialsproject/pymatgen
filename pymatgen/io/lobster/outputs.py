...
class HamiltonMatrices:
    """
    Class to read hamiltonMatrices.lobster files generated by LOBSTER.
    ...
    Attributes:
        ...
        average_onsite_energies (dict): dict with average onsite elements energies for all k-points with keys as
                                        basis used in the LOBSTER computation.
        real_hamilton_matrices (list[np.arrays]) : dict with matrix at each k-point with k-point and spin as keys
    """

    def __init__(self, e_fermi, filename: str = "hamiltonMatrices.lobster"):
        """
        Args:
            filename: filename for the hamiltonMatrices file, typically "hamiltonMatrices.lobster".
            e_fermi: fermi level in eV for the structure
        """
        # hamiltonMatrices
        with zopen(filename, "rt") as f:
            data = f.readlines()
        if len(data) == 0:
            raise OSError("hamiltonMatrices file contains no data.")
        ...
        start_inxs = []
        end_inxs = []
        for i, line in enumerate(data):  # get indices of real part of matrix for each k point
            line = line.strip()
        ...
        for start, end in zip(start_inxs, end_inxs):  # extracts matrix data and store (onsite) diagonal elements
        ...
        average_onsite_hamil = np.array(self.onsite_energies, dtype=float).mean(axis=0)  # get average row wise
        # create a dict with basis functions as keys
        self.average_onsite_energies = dict(zip(elements, average_onsite_hamil))
