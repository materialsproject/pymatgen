...
class Header(MSONable):
    ...
    def to_dict(self):
        ...
        """
        header_dict={}
        header_dict['@module'] = self.__class__.__module__
        header_dict['@class'] = self.__class__.__name__
        header_dict['comment'] = self._comment
        header_dict['source'] = self.source
        header_dict['structure'] = self._struct.to_dict
    ...
    def from_string(header_str):
        ...
        #generate structure object so it is not possible to generate header object
        #and routine ends
...
class FeffAtoms(MSONable):
    ...
    def to_dict(self):
        ...
        """
        atoms_dict={}
        atoms_dict['@module'] = self.__class__.__module__
        atoms_dict['@class'] = self.__class__.__name__
        atoms_dict['structure'] = self._struct.to_dict
        atoms_dict['central_atom'] = self._central_atom
        atoms_dict['comment'] = self.comment
...
class FeffTags(dict):
    ...
    def __setitem__(self, key, val):
        ...
        Arg:
            ...
            value:
                value associated with key in dictionary
        """
    ...
    def from_file(filename="feff.inp"):
        ...
        Args:
            filename:
                Filename for dither PARAMETER or feff.inp file
...
class FeffPot(MSONable):
    ...
    def to_dict(self):
        ...
        """
        pot_dict={}
        pot_dict['@module'] = self.__class__.__module__
        pot_dict['@class'] = self.__class__.__name__
        pot_dict['structure'] = self._struct.to_dict
        pot_dict['central_atom'] = self._central_atom
        pot_dict['comment'] = self.comment
...
class FeffLdos(MSONable):
    ...
    def from_file(filename1='feff.inp', filename2='ldos'):
        ...
        for i in range(1, len(pot_dict) + 1):
            if len(str(i)) == 1:
                ldos[i] = np.loadtxt(ldos_filename + "0" + str(i) +
                                          ".dat")
            else:
                ldos[i] = np.loadtxt(ldos_filename + str(i) + ".dat")
    ...
    def charge_transfer_from_file(filename1, filename2):
        ...
        for i in range(1, len(dicts[0])+1):
            if len(str(i)) == 1:
                with zopen("{}0{}.dat".format(filename2, i), "r") \
                    ...
                    cht[pot_dict[i]] = {'s':s,'p':p,'d':d,'f':f1,'tot':tot}
            else:
...
class Xmu(MSONable):
    ...
    def __init__(self, xmu_dict):
        """
        Args:
            xmu_dict:

                dictionary of input filename and numpy data
        """

        self._input_file = xmu_dict['feff.inp']
        self._data = xmu_dict['xmu']
    ...
    def to_dict(self):
        ...
        xmu_dict={}
        xmu_dict['@module']  = self.__class__.__module__
        xmu_dict['@class']   = self.__class__.__name__
        xmu_dict['feff.inp'] = self._input_file
        xmu_dict['xmu']      = self._data
        xmu_dict['energies'] = self.energies
        xmu_dict['across']   =  self.across_section
        xmu_dict['scross']   = self.scross_section
        xmu_dict['atom']     = self.absorbing_atom
        xmu_dict['edge']     =  self.edge
        xmu_dict['source']   = self.source
        xmu_dict['calc']     = self.calc
        xmu_dict['formula']  = self.material_formula
