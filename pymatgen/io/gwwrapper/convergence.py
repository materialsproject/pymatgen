...
def print_plot_line(function, popt, xs, ys, name, extra=''):
    ...
    if function is exponential:
        line = "plot %s + %s * %s ** -x, 'convdat.%s w pointsize 4 lt 0', %s" % (popt[0], popt[1], popt[2], idp, popt[0])
        #print 'plot ', popt[0], ' + ', popt[1], "* ", popt[2], " ** -x," "'"+'convdat.'+idp+"'"
    elif function is reciprocal:
        line = "plot %s + %s / x**%s, 'convdat.%s w pointsize 4 lt 0', %s" % (popt[0], popt[1], popt[2], idp, popt[0])
