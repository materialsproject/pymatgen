...
class TestLammpsInputSet(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.lammps_input_set = LammpsInputSet.from_file("test", template_file,
                                                         self.settings,
                                                         lammps_data=data_file,
                                                         data_filename=self.data_filename,
                                                         is_forcefield=True)

    def test_input(self):
        self.assertEqual(self.lammps_input_set.lammps_input["read_data"],
                         self.data_filename)
        for k, v in self.settings.items():
            self.assertEqual(self.lammps_input_set.lammps_input[k], v)
