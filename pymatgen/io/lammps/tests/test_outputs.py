...
class LammpsDumpTest(unittest.TestCase):
    ...
    def test_from_string(self):
        ...
        np.testing.assert_array_equal(
            self.rdx.data.columns, ["id", "type", "xs", "ys", "zs"]
        )
        ...
        np.testing.assert_array_equal(
            self.tatb.data.columns, ["id", "type", "q", "x", "y", "z"]
        )
...
class FuncTest(unittest.TestCase):
    ...
    def test_parse_lammps_log(self):
        ...
        comb0 = comb[0]
        np.testing.assert_array_equal(
            ["Step", "Temp", "TotEng", "PotEng", "E_vdwl", "E_coul"], comb0.columns
        )
        ...
        # ehex run #1
        np.testing.assert_array_equal(
            ["Step", "Temp", "E_pair", "E_mol", "TotEng", "Press"], ehex0.columns
        )
        ...
        # ehex run #2
        np.testing.assert_array_equal(
            ["Step", "Temp", "c_Thot", "c_Tcold"], ehex1.columns
        )
        ...
        # ehex run #3
        np.testing.assert_array_equal(
            ["Step", "Temp", "c_Thot", "c_Tcold", "v_tdiff", "f_ave"], ehex2.columns
        )
