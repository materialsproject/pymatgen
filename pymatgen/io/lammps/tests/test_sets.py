...
class LammpsMinimizationTest(PymatgenTest):
    ...
    def test_get_input_set(self):
        ...
        self.assertListEqual(
            list(lmp_min.data.as_dict().keys()), list(LammpsData.from_structure(self.structure).as_dict().keys())
        )
        self.assertArrayAlmostEqual(
            lmp_min.data.as_dict()["atoms"].values, LammpsData.from_structure(self.structure).as_dict()["atoms"].values
        )

        self.assertListEqual(
            lmp_min.inputfile.input_settings,
            [
                [
                    "Stage 1",
                    [
