...
class mCifWriter:
    def __init__(self, struct, symprec=None):
        """
        A wrapper around CifFile to write magnetic CIF files from pymatgen structures.
        ...
        comp = struct.composition
        no_oxi_comp = comp.element_composition
        block["_parent_space_group.name_H-M_alt"] = spacegroup[0]
        block["_parent_space_group.IT_number"] = spacegroup[1]
        # _parent_space_group.transform_Pp_abc  'a,b,c;0,0,0'

        #loops.append(["_parent_propagation_vector.id",
        #               "_parent_propagation_vector.kxkykz"]
        # k1 [0 0 0]

        # _parent_space_group.child_transform_Pp_abc  '1/2a-1/2b,1/2a+1/2b,c;1/4,1/4,0'
        # _space_group_magn.transform_BNS_Pp_abc  'a,b,c;0,0,0'

        block["_space_group_magn.number_BNS"] = ".".join( [str(i) for i in mspg[1] ] )
        block["_space_group_magn.name_BNS"] = mspg[0]

        block["_point_group_magn.number"] = ".".join( [str(i) for i in mpg[1] ] )
        block["_point_group_magn.name"] = mpg[0]
        ...
        if symprec is None:
            ...
            g = SpaceGroup.from_int_number(spacegroup[1])
            ops = [op.as_xyz_string() for op in g.symmetry_ops]
        ...
        #loops.append(["_space_group_symop_magn_operation.id",
        #             "_space_group_symop_magn_operation.xyz"])  # 1 x,y,z,+1

        #loops.append(["_space_group_symop_magn_centering.id,
        #              "_space_group_symop_magn_centering.xyz"])  # 1 x,y,z,+1
