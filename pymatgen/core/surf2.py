...
class Slab(Structure):
    ...
    def __init__(self, normal, scale_factor, structure, miller_index,
...
def generate_slab(initial_structure, miller_index, normal, scale_factor, oriented_unit_cell, dist, min_slab_size, min_vac_size, shift, lll_reduce, standardize):
    """
    ...
    new_sites = []
    for site in slab:
        if shift <= np.dot(site.coords, normal) < nlayers*dist +\
                shift:
    ...
    return Slab(normal, scale_factor, slab, miller_index,
                initial_structure, min_slab_size, min_vac_size)
...
class SurfaceGenerator():
    ...
    def get_all_slab(self, thresh=0.00001, crit ='distance'):
        ...
        for i in xrange(len(term_index)):
            ...
            term_sites.append(term_slab.frac_coords[term_index[i]][2])

        slab_list = []
        for shift in term_sites:
            slab_list.append(generate_slab(self.parent, self.miller_index, self.normal, self.slab_scale_factor, self.oriented_unit_cell, self.dist, self.min_slab_size,
                                           self.min_vac_size, shift, self.lll_reduce, self.standardize))
