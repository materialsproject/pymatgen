...
0.000 0.000 10.000""",
    ...
    def test_selling_vector(self):
        a1 = 10
        np.testing.assert_array_equal(Lattice.selling_vector(Lattice.cubic(a1)), np.array([0, 0, 0, -a1**2, -a1**2, -a1**2]))
        a2, c2 = 5, 8
        np.testing.assert_array_almost_equal(Lattice.selling_vector(Lattice.tetragonal(a2, c2)), np.array([0, 0, 0, -a2**2, -a2**2, -c2**2]))
        a3, b3, c3 = 4, 6, 7
        np.testing.assert_array_almost_equal(Lattice.selling_vector(Lattice.orthorhombic(a3, b3, c3)), np.array([0, 0, 0, -b3**2, -a3**2, -c3**2]))
