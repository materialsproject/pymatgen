...
class MoleculeOptimizeTrajectoryTest(PymatgenTest):
    def setUp(self):
        out = QCOutput(os.path.join(PymatgenTest.TEST_FILES_DIR, "molecules", "new_qchem_files", "ts.out"))
    ...
    def test_single_index_slice(self):
        assert all([self.traj[i] == self.molecules[i] for i in range(0, len(self.molecules))])

    def test_slice(self):
        sliced_traj = self.traj[0:2]
        sliced_traj_from_mols = MoleculeOptimizeTrajectory.from_molecules(self.molecules[0:2])
    ...
    def test_list_slice(self):
        sliced_traj = self.traj[[1, 3]]
        sliced_traj_from_mols = MoleculeOptimizeTrajectory.from_molecules([self.molecules[i] for i in [1, 3]])
    ...
    def test_conversion(self):
        ...
        assert all([mol == self.molecules[i] for i, mol in enumerate(self.traj)])
    ...
    def test_site_properties(self):
        ...
        traj = MoleculeOptimizeTrajectory(species, coords, charge, spin, site_properties=props)
    ...
    def test_extend(self):
        ...
        traj = copy.deepcopy(self.traj)
        incompatible_traj = MoleculeOptimizeTrajectory(species, coords, charge, spin)
        incompatible_test_success = False
        try:
            traj.extend(incompatible_traj)
        except Exception:
    ...
    def test_extend_site_props(self):
        ...
        traj_1 = MoleculeOptimizeTrajectory(species, coords, charge, spin, site_properties=props_1)
        traj_2 = MoleculeOptimizeTrajectory(species, coords, charge, spin, site_properties=props_2)
        traj_3 = MoleculeOptimizeTrajectory(species, coords, charge, spin, site_properties=props_3)
        traj_4 = MoleculeOptimizeTrajectory(species, coords, charge, spin)
