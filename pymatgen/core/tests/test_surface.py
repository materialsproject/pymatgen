...
class center_slabTests(PymatgenTest):

    def setUp(self):
        self.lifepo4 = self.get_structure("LiFePO4")

    def test_center_slan(self):
        """
        Test out center of mass. 3 slabs: half/half slab/vacuum,
        0.25/0.75 slab/vacuum and 0.75/0.25 slab/vacuum. For
        each of these cases, test out Scenario 1, 2, 3 and centered
        """

        for sizes in [[20, 20], [10, 30], [30, 10]]:
            slab = SlabGenerator(self.lifepo4, (2, 1, 0), sizes[0], sizes[1],
                                 center_slab=True).get_slab()
            sorted_sites = sorted(slab, key=lambda site: site.frac_coords[2])
            slab = Structure.from_sites(sorted_sites)
            ...
            all_true = []
            for site in s:
                all_true.append(all([s.lattice.c >= nn[1] >= 0 for nn
                                     in s.get_neighbors(site, 4.7)]))
            self.assertTrue(all(all_true))

            # test scenario 1
            s = slab.copy()
            s.translate_sites(all_indices, [0, 0, -s[0].frac_coords[2] / 2])
            s = center_slab(s)
            all_true = []
            for site in s:
                all_true.append(all([s.lattice.c >= nn[1] >= 0 for nn
            ...
            # test scenario 2
            s = slab.copy()
            s.translate_sites(all_indices, [0, 0, -s[0].frac_coords[2]])
            s = center_slab(s)
            all_true = []
            for site in s:
                all_true.append(all([s.lattice.c >= nn[1] >= 0 for nn
            ...
            # test scenario 3
            s = slab.copy()
            t = (s[-1].frac_coords[2] - s[0].frac_coords[2]) / 2
            s.translate_sites(all_indices, [0, 0, -(s[0].frac_coords[2] + t)])
            s = center_slab(s)
            all_true = []
            for site in s:
                all_true.append(all([s.lattice.c >= nn[1] >= 0 for nn
