...
class MaterialsProjectDFTMixingScheme(Compatibility):
    ...
    def get_adjustments(self, entry, mixing_state_data: pd.DataFrame = None):
        ...
        if (entry.entry_id not in mixing_state_data["entry_id_1"]) and (
            entry.entry_id not in mixing_state_data["entry_id_2"]
        ):
            raise CompatibilityError(
        ...
        if (entry.energy_per_atom not in mixing_state_data["energy_1"]) and (
            entry.energy_per_atom not in mixing_state_data["energy_2"]
        ):
            raise CompatibilityError(
                f"WARNING! Discarding {run_type} entry {entry.entry_id} for {entry.composition.formula} "
