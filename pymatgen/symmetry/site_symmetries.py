...
def get_site_symmetries(struc, precision = 0.1):
    ...
    for site1 in range(len(struc.sites)):
        ...
        pointops.append([])
        for site2 in range(len(strucs.sites)):
...
def get_shared_symmetry_operations(strucs, symops, tol = 0.1):
    ...
    for site1 in range(numsites):
        for site2 in range(numsites):
            ...
            for op1 in range(len(symops[site1])):
                for op2 in range(len(symops[site2])):
                    if np.allclose(symops[site1][op1].rotation_matrix, symops[site2][op2].rotation_matrix):
                        sharedops[site1][site2].append(symops[site1][op1])
