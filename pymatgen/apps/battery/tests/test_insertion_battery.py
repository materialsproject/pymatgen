...
class InsertionElectrodeTest(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(PymatgenTest.TEST_FILES_DIR, "MgVO_batt.json.gz")) as file:
            self.entries_MVO = json.load(file, cls=MontyDecoder)

        with open(os.path.join(PymatgenTest.TEST_FILES_DIR, "Mg_batt.json.gz")) as file:
            self.entry_Mg = json.load(file, cls=MontyDecoder)

        with open(os.path.join(PymatgenTest.TEST_FILES_DIR, "CaMoO2_batt.json.gz")) as f:
