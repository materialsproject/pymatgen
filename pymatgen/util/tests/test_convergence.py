...
class ConvergenceTest(PymatgenTest):
    def test_determine_convergence(self):
        ...
        ys = [4, 5, 6, 6, 6, 6]
        assert determine_convergence(xs, ys, name="name", tol=0.1, plots=False) == pytest.approx(
            [True, 4, 6, 3, 6.0, 0.091269841269839724]
        )
        # another converging example
        ys = [1 / 1, 1 / 2, 1 / 3, 1 / 4, 1 / 5, 1 / 6]
        assert determine_convergence(xs, ys, name="name", tol=0.3, plots=False) == pytest.approx(
