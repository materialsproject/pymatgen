...
from pymatgen.phasediagram.entries import PDEntryIO, PDEntry, GrandPotPDEntry, TransformedPDEntry
...
class PDEntryTest(unittest.TestCase):
    ...
    def test_to_from_dict(self):
        ...
        self.assertEquals(entry.energy_per_atom, 53.0 / 4, "Wrong energy per atom!")
...
class TransformedPDEntryTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.transformed_entry = TransformedPDEntry({DummySpecie('Xa'):1, DummySpecie("Xb"):1},
                                                    entry)

    def test_get_energy(self):
        self.assertEquals(self.transformed_entry.energy, 53, "Wrong energy!")
        self.assertEquals(self.transformed_entry.original_entry.energy, 53.0, "Wrong energy per atom!")

    def test_get_energy_per_atom(self):
        self.assertEquals(self.transformed_entry.energy_per_atom, 53.0 / 2, "Wrong energy per atom!")
    ...
    def test_is_element(self):
        self.assertFalse(self.transformed_entry.is_element, "Wrongly assigned as element!")

    def test_to_from_dict(self):
        d = self.transformed_entry.to_dict
        entry = TransformedPDEntry.from_dict(d)
        self.assertEquals(entry.name, 'LiFeO2', "Wrong name!")
        self.assertEquals(entry.energy_per_atom, 53.0 / 2, "Wrong energy per atom!")
