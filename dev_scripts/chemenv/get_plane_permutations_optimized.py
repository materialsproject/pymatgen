...
if __name__ == '__main__':
    ...
    option_parser.add_option("-v", "--printing_volume",
                             action="store", type=int, default=1,
                             help="Printing volume (0, 1 or 2)",
                             dest="printing_volume")
    option_parser.add_option("-p", "--permutations_setup",
                             action="store", type=str, default="y50",
    ...
    while True:
        ...
        while True:
            test = raw_input('Get the explicit optimized permutations for geometry "{}" (symbol : "{}") ? '
        ...
        for ialgo, algo in enumerate(cg.algorithms):
            ...
            if permutations_setup_type == 'all':
                ...
                if npermutations >= factorial(cg.coordination_number):
                    ...
                    perms_iterator = random_permutations_iterator(initial_permutation=myindices,
            ...
            for indices_perm in perms_iterator:
                ...
                for iplane, plane_point_indices in enumerate(all_planes_point_indices):
                    ...
                    prt2(string='  permutation {} gives csm={:.6f}'.format('-'.join(str(pp) for pp in this_plane_sep_perm),
                                                                           csms[imin]),
                         printing_volume=printing_volume)
            ...
            print('For ialgo {:d} (plane_points : {}, '
                  ...
                  'are :'.format(ialgo,
                                 ...
                                 '[{}]'.format(', '.join([str(pp) for pp in algo.point_groups[0]])),
                                 '[{}]'.format(', '.join([str(pp) for pp in algo.point_groups[1]])),
                                 len(perms_used), len(algo.permutations),
                                 str(original_nexplicit_optimized_perms[ialgo]),
                                 str(original_nexplicit_perms[ialgo])
                                 ))
            ...
            print('')
            test = raw_input('Set optimized permutations for algorithm {:d} ? ("y" to confirm)'.format(ialgo))
            if test == 'y':
                algo.explicit_optimized_permutations = explicit_optimized_permutations

        test = raw_input('Save coordination geometry "{}" (symbol "{}") and new explicit and optimized permutations ? '
