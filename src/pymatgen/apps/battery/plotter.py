    ...
    def get_plotly_figure(
        ...
        **kwargs,
        ...
        for key, electrode in self._electrodes.items():
            ...
            working_ion_symbols.add(electrode.working_ion.symbol)
            formula.add(electrode.framework_formula)
            # Plot the raw stair-step data; consecutive points sharing an x value give the
            # vertical drop between plateaus (matches the matplotlib renderer).
            data.append(go.Scatter(x=x, y=y, mode="lines", name=key, hovertemplate=hover_temp))
