...
class SupercellTransformation(AbstractTransformation):
    ...
    def from_scaling_factors(
        cls, scale_a: float = 1, scale_b: float = 1, scale_c: float = 1
    ) -> Self:
    ...
    def from_boundary_distance(
        ...
        min_expand = np.int8(
            min_boundary_dist
            / np.array([structure.lattice.d_hkl(plane) for plane in np.eye(3)])
        )
        ...
        if allow_rotation and sum(min_expand != 0) > 1:
            ...
            min_expand_scaled = np.int8(
                min_boundary_dist
                / np.array([struct_scaled.lattice.d_hkl(plane) for plane in np.eye(3)])
            )
...
class SubstitutionTransformation(AbstractTransformation):
    ...
    def __init__(
        self,
        species_map: (
            dict[SpeciesLike, SpeciesLike | dict[SpeciesLike, float]]
            | list[tuple[SpeciesLike, SpeciesLike]]
        ),
    ) -> None:
        """
        Args:
    ...
    def apply_transformation(self, structure: Structure) -> Structure:
        ...
        for k, v in self._species_map.items():
            value = (
                {get_el_sp(x): y for x, y in v.items()}
    ...
    def __repr__(self):
        return "Substitution Transformation :" + ", ".join(
            [f"{k}->{v}" for k, v in self._species_map.items()]
        )
...
class OrderDisorderedStructureTransformation(AbstractTransformation):
    ...
    def apply_transformation(
        ...
        if self.symmetrized_structures and not isinstance(
            ...
            structure = SpacegroupAnalyzer(
                ...
                **{
                    k: getattr(self, k)
                    for k in ("symprec", "angle_tolerance")
        ...
        for group in equivalent_sites:
            ...
            for key, val in total_occupancy.items():
                if key == initial_sp:
                    continue
                oxi_ratio = (
                    key.oxi_state / initial_sp.oxi_state if initial_sp.oxi_state else 0
        ...
        if self.algo == self.ALGO_RANDOM:
            ...
            if return_ranked_list:
                return [
                    {"energy": 0.0, "energy_above_minimum": 0.0, "structure": s}
                    for s in rand_structures[:n_to_return]
                ]
...
class ConventionalCellTransformation(AbstractTransformation):
    ...
    def __init__(
        self, symprec: float = 0.01, angle_tolerance=5, international_monoclinic=True
    ):
        """
        Args:
...
class DiscretizeOccupanciesTransformation(AbstractTransformation):
    ...
    def __init__(
        self, max_denominator=5, tol: float | None = None, fix_denominator=False
    ):
        """
        Args:
            max_denominator:
        ...
        for sp in species:
            for k in sp:
                ...
                if self.fix_denominator:
                    new_occ = (
                        np.around(old_occ * self.max_denominator) / self.max_denominator
                    )
...
class ScaleToRelaxedTransformation(AbstractTransformation):
    ...
    def __init__(self, unrelaxed_structure, relaxed_structure, species_map=None):
        ...
        self.params_percent_change = [
            relax_params[idx] / unrelax_params[idx] for idx in range(len(relax_params))
        ]
    ...
    def apply_transformation(self, structure):
        ...
        new_lattice = Lattice.from_parameters(
            *(
                param * self.params_percent_change[idx]
...
def _sample_random_manipulation(
    manipulation, rng, manipulated
) -> list[tuple[int, SpeciesLike]]:
...
def _get_manipulation(
    manipulations: list, rng: Generator, max_attempts, seen: set[tuple]
) -> tuple:
