...
class SuperTransformation(AbstractTransformation):
    ...
    def apply_transformation(
        ...
        if not return_ranked_list:
            raise ValueError(
                "SuperTransformation has no single best structure output. Must use return_ranked_list"
            )
        structures = []
...
class EnumerateStructureTransformation(AbstractTransformation):
    ...
    def apply_transformation(
        ...
        contains_oxidation_state = all(
            hasattr(sp, "oxi_state") and sp.oxi_state != 0 for sp in structure.elements
        )
        ...
        if self.max_disordered_sites:
            ...
            if n_disordered > self.max_disordered_sites:
                raise ValueError(
                    f"Too many disordered sites! ({n_disordered} > {self.max_disordered_sites})"
                )
        ...
        for max_cell_size in max_cell_sizes:
            ...
            try:
                ...
                warnings.warn(
                    f"Unable to enumerate for {max_cell_size = }", stacklevel=2
                )
        ...
        def _get_stats(struct):
            ...
            if contains_oxidation_state and self.sort_criteria == "ewald":
                ...
                transformation = tuple(
                    tuple(round(cell) for cell in row) for row in transformation
                )
        ...
        all_structures = Parallel(n_jobs=self.n_jobs)(
            delayed(_get_stats)(struct) for struct in structures
        )
...
class SubstitutionPredictorTransformation(AbstractTransformation):
    ...
    def apply_transformation(
        ...
        if not return_ranked_list:
            raise ValueError(
                "SubstitutionPredictorTransformation doesn't support returning 1 structure"
            )
...
class MagOrderingTransformation(AbstractTransformation):
    ...
    def __init__(
        self, mag_species_spin, order_parameter=0.5, energy_model=None, **kwargs
    ):
        """
        Args:
    ...
    def _add_dummy_species(structure, order_parameters):
        ...
        dummy_species = [
            ...
            for symbol, constraint in zip(
                dummy_species_symbols, order_parameters, strict=True
            )
        ]
        ...
        for site in dummy_struct:
            ...
            if satisfies_constraints.count(True) > 1:
                ...
                # one constraint
                raise ValueError(
                    f"Order parameter constraints conflict for site: {site.specie}, {site.properties}"
                )
    ...
    def apply_transformation(
        ...
        all_structs = trafo.apply_transformation(
        ...
        if isinstance(all_structs, Structure):
            ...
            for idx, struct in enumerate(all_structs):
                all_structs[idx]["structure"] = self._remove_dummy_species(
                    struct["structure"]
                )  # type: ignore[index]
                all_structs[idx]["structure"] = self._add_spin_magnitudes(
                    struct["structure"]
        ...
        for _, group in groupby(
            sorted((dct["structure"] for dct in all_structs), key=key), key
    ...
    def apply_transformation(
        ...
        for sp in compatible_species:
            ...
            if sp.oxi_state == ox:
                ...
                logger.info(
                    f"Doping {sp} for {self.dopant} + {codopant} at level {1 / nsp:.3f}"
                )
                ...
                if sp_to_remove == sp:
                    ...
                    logger.info(
                        f"Doping {nsp_to_remove} {sp} with {n_dopant} {self.dopant}."
                    )
                    supercell.replace_species(
                        {
                            sp: {
            ...
            structs = trafo.apply_transformation(
...
class CubicSupercellTransformation(AbstractTransformation):
    ...
    def apply_transformation(self, structure: Structure) -> Structure:
        ...
        if not self.allow_orthorhombic:
            ...
            while sc_not_found:
                ...
                length_vecs, n_atoms, superstructure, self.transformation_matrix = (
                    self.get_possible_supercell(lat_vecs, structure, target_sc_lat_vecs)
                )
        ...
        for size_a, size_b, size_c in combined_list:
            target_sc_lat_vecs = np.array(
                [[size_a, 0, 0], [0, size_b, 0], [0, 0, size_c]]
            )
            length_vecs, n_atoms, superstructure, self.transformation_matrix = (
                self.get_possible_supercell(lat_vecs, structure, target_sc_lat_vecs)
            )
    ...
    def check_constraints(self, length_vecs, n_atoms, superstructure):
        ...
        return bool(
            ...
            and (
                ...
                or np.all(
                    np.absolute(np.array(superstructure.lattice.angles) - 90)
    ...
    if (~arr_rounded.any(axis=0)).any():
        ...
        for zero_col_idx in zero_col_idxs:
            ...
            for idx in row_idx_to_fix:
                arr_rounded[idx, zero_col_idx] = round_away_from_zero(
                    arr[idx, zero_col_idx]
                )
...
class SQSTransformation(AbstractTransformation):
    ...
    def _get_disordered_substructure(struct_disordered):
        ...
        idx_to_remove = [
            idx for idx, site in enumerate(disordered_substructure) if site.is_ordered
        ]
    ...
    def _sqs_cluster_estimate(
        struct_disordered, cluster_size_and_shell: dict[int, int] | None = None
    ):
    ...
    def apply_transformation(
        ...
        if return_ranked_list and self.instances is None:
            raise ValueError(
                f"{type(self).__name__} has no instances, so cannot return a ranked list"
            )
        if (
            ...
            raise ValueError(
                f"{return_ranked_list=} cannot be greater than {self.instances=}"
            )
    ...
    @staticmethod
    def _get_unique_best_sqs_structs(
        ...
        if not return_ranked_list:
            ...
            # reduce structure
            if reduction_algo:
                return_struct = return_struct.get_reduced_structure(
        ...
        for dct in sqs.allsqs:
            ...
            if (not best_only) or (
                best_only and dct["objective_function"] == sqs.objective_function
            ):
        ...
        to_return = [
            {"structure": struct, "objective_function": struct.objective_function}
        ...
        for dct in to_return:
            ...
            # reduce structure
            if reduction_algo:
                dct["structure"] = dct["structure"].get_reduced_structure(
                    reduction_algo=reduction_algo
                )

        # because bools are subclasses of ints, cannot just check that returned_ranked_list is int
        if isinstance(return_ranked_list, int) and not isinstance(
            return_ranked_list, bool
        ):
...
class MonteCarloRattleTransformation(AbstractTransformation):
    ...
    def __init__(
        self, rattle_std: float, min_distance: float, seed: int | None = None, **kwargs
    ):
        """
        Args:
