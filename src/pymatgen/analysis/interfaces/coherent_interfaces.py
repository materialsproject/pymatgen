...
class CoherentInterfaceBuilder:
    ...
    def _find_terminations(self):
        ...
        if self.label_index:
            film_terminations = [
                label_termination(slab, self.termination_ftol[0], t_idx) for t_idx, slab in enumerate(film_slabs, start=1)
            ]
        else:
        ...
        if self.label_index:
            sub_terminations = [
                label_termination(slab, self.termination_ftol[1], t_idx) for t_idx, slab in enumerate(sub_slabs, start=1)
            ]
        else:
