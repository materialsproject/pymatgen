...
class ThermalDisplacementMatrices(MSONable):
    ...
    def __init__(
        ...
        Args:
            ...
            thermal_displacement_matrix_cif: 2D numpy array including the thermal_displacement matrix Ucif format
                ...
                convention similar to "thermal_displacement_matrices.yaml" in phonopy.
        """
        self.thermal_displacement_matrix_cart = np.asarray(
            thermal_displacement_matrix_cart
        )
        self.structure = structure
        self.temperature = temperature
        if thermal_displacement_matrix_cif is not None:
            self.thermal_displacement_matrix_cif: None | NDArray = np.asarray(
        ...
        if self.thermal_displacement_matrix_cif is not None:
            self.thermal_displacement_matrix_cif_matrixform = (
                ThermalDisplacementMatrices.get_full_matrix(
    ...
    def write_cif(self, filename: str) -> None:
        ...
        with open(filename, mode="a", encoding="utf-8") as file:
            ...
            file.writelines(
                ...
                for idx, (site, matrix) in enumerate(
                    zip(self.structure, self.Ucif, strict=True)
                )
            )
    ...
    def compute_directionality_quality_criterion(
        ...
        for spec1, spec2 in zip(
            self.structure.species, other.structure.species, strict=True
        ):
            if spec1 != spec2:
                raise ValueError(
        ...
        for self_Ucart, other_Ucart in zip(
            ...
            invUcart_eig_self, invUcart_eigv_self = np.linalg.eig(
                np.linalg.inv(self_Ucart)
            )
            invUcart_eig_other, invUcart_eigv_other = np.linalg.eig(
                np.linalg.inv(other_Ucart)
            )
    ...
    def visualize_directionality_quality_criterion(
        ...
        with open(filename, mode="w", encoding="utf-8") as file:
            ...
            for vectors in result:
                ...
                file.write(
                    f"    {vector_count} {vector0_x} {vector0_y} {vector0_z} 0\n"
                )
                ...
                vector_count += 1
                file.write(
                    f"    {vector_count} {vector1_x} {vector1_y} {vector1_z} 0\n"
                )
                file.write(f"    {site_count} 0 0 0 0\n")
                vector_count += 1
                site_count += 1
    ...
    def from_Ucif(
        ...
        """
        # get matrix form
        thermal_displacement_matrix_cif_matrix_form = (
            ThermalDisplacementMatrices.get_full_matrix(thermal_displacement_matrix_cif)
        )
        ...
        thermal_displacement_matrix_cart = (
            ThermalDisplacementMatrices.get_reduced_matrix(np.array(Ucart))
        )
    ...
    def from_structure_with_site_properties_Ucif(
        ...
        # U11, U22, U33, U23, U13, U12
        Ucif_matrix = [
            [site.properties[f"U{idx}_cif"] for idx in (11, 22, 33, 23, 13, 12)]
        ...
        for data in cif.data.values():
            ...
            for idx in range(len(data["_atom_site_label"])):
                try:
                    ...
                    # label.
                    symbol = CifParser(filename)._parse_symbol(
                        data["_atom_site_type_symbol"][idx]
                    )
                except KeyError:
                    symbol = CifParser(filename)._parse_symbol(
                        data["_atom_site_label"][idx]
                    )
                if not symbol:
                    continue
