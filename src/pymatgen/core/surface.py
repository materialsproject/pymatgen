...
class Slab(Structure):
    ...
    def __str__(self) -> str:
        ...
        for idx, site in enumerate(self):
            outs.append(
                f"{idx + 1} {site.species_string} {' '.join(f'{j:0.6f}'.rjust(12) for j in site.frac_coords)}"
            )
    ...
    def is_symmetric(self, symprec: float = 0.1) -> bool:
        ...
        return bool(
            ...
            or any(
                np.all(op.rotation_matrix[2] == np.array([0, 0, -1])) for op in symm_ops
            )
        )
    ...
    def get_surface_sites(self, tag: bool = False) -> dict[str, list]:
        ...
        for idx, site in enumerate(self):
            ...
            try:
                ...
                cn = float(
                    f"{round(voronoi_nn.get_cn(self, idx, use_weights=True), 5):.5f}"
                )
    ...
    def get_tasker2_slabs(
        ...
        n_layers_slab = round(
            (sorted_csites[-1].c - sorted_csites[0].c) * n_layers_total
        )
        ...
        for surface_site, shift in [
            ...
            grouped = [
                list(sites)
                for k, sites in itertools.groupby(to_move, key=get_equi_index)
            ]
            ...
            combinations = [
                list(itertools.combinations(g, len(g) // 2)) for g in grouped
            ]
            ...
            for selection in itertools.product(*combinations):
                ...
                for struct_matcher in to_move:
                    ...
                    for group in selection:
                        ...
                        frac_coords.append(
                            struct_matcher.frac_coords + np.array([0, 0, shift])
                        )

                # sort by species to put all similar species together.
                sp_fcoord = sorted(
                    zip(species, frac_coords, strict=True), key=lambda x: x[0]
                )
    ...
    def symmetrically_remove_atoms(self, indices: list[int]) -> None:
        ...
        if len(equi_sites) == len(indices):
            ...
            warnings.warn(
                "Equivalent sites could not be found for some indices. Surface unchanged.",
                stacklevel=2,
            )
...
def center_slab(slab: Structure) -> Structure:
    ...
    for site in slab:  # DEBUG (@DanielYang59): Slab position changes during loop?
        ...
        if any(
            nn[1] >= slab.lattice.c for nn in slab.get_neighbors(site, cutoff_radius)
        ):
...
def get_slab_regions(
    ...
    if frac_coords:
        ...
        upper_fcoords: list = [
            ...
            for site in slab
            if all(
                nn.index not in all_indices for nn in slab.get_neighbors(site, blength)
            )
        ]
        coords: list = (
            copy.copy(frac_coords) if frac_coords else copy.copy(last_frac_coords)
        )
...
class SlabGenerator:
    ...
    def __init__(
        ...
        def reduce_vector(vector: tuple[int, ...]) -> tuple[int, ...]:
            ...
            return cast(
                "tuple[int, int, int]", tuple(int(idx / divisor) for idx in vector)
            )
        ...
        def add_site_types() -> None:
            ...
            if (
                ...
                initial_structure.add_site_property(
                    "bulk_wyckoff", spg_analyzer.get_symmetry_dataset().wyckoffs
                )
                initial_structure.add_site_property(
                    "bulk_equivalent",
    ...
    def get_slab(
        ...
        height_per_layer = round(
            height / self.parent.lattice.d_hkl(self.miller_index), 8
        )
        ...
        struct: Structure = Structure(
            new_lattice, species * n_layers_slab, all_coords, site_properties=props
        ...
        if self.primitive:
            ...
            ouc = (
                ouc
                if (slab_l.a == ouc.lattice.a and slab_l.b == ouc.lattice.b)
                else self.oriented_unit_cell
            )
    ...
    def get_slabs(
        ...
        def gen_possible_terminations(ftol: float) -> list[float]:
            ...
            clst_loc: dict[Any, float] = {
                clst: frac_coords[idx][2] for idx, clst in enumerate(clusters)
            }

            # Wrap all clusters into the unit cell ([0, 1) range)
            possible_clst: list[float] = [
                coord - math.floor(coord) for coord in sorted(clst_loc.values())
            ]
                    ...
                    if sp1 in site.species:
                        for nn in self.oriented_unit_cell.get_neighbors(
                            ...
                            if sp2 in nn.species:
                                ...
                                if z_range[1] > 1:
                                    z_ranges.extend(
                                        [(z_range[0], 1), (0, z_range[1] - 1)]
                                    )

                                # When z coordinate is below the lower boundary
                                elif z_range[0] < 0:
                                    z_ranges.extend(
                                        [(0, z_range[1]), (z_range[0] + 1, 1)]
                                    )

                                # Neglect overlapping positions
                                elif not math.isclose(
                                    z_range[0], z_range[1], abs_tol=ztol
                                ):
        ...
        if filter_out_sym_slabs:
            ...
            if symmetrize:
                matcher_sym = StructureMatcher(
                    ltol=tol, stol=tol, primitive_cell=False, scale=False
                )
                final_slabs = [
                    group[0] for group in matcher_sym.group_structures(final_slabs)
                ]
        else:
            final_slabs = slabs
                ...
                if site.species_string == ele_ref:
                    ...
                    if ref_cn not in cn_dict[ele_ref]:
                        ...
                        to_move = [
                            nn[2]
                            for nn in neighbors
                            if nn[0].species_string == ele_other
                        ]
                        to_move.append(idx)
    ...
    def move_to_other_side(
        ...
        slab.translate_sites(
            top_site_index, vector=[0, 0, -frac_dist], frac_coords=True
        )
        slab.translate_sites(
            bottom_site_index, vector=[0, 0, frac_dist], frac_coords=True
        )
        ...
        for surface in ("top", "bottom"):
            ...
            while not is_sym:
                ...
                if len(slab) <= len(self.parent):
                    warnings.warn(
                        "Too many sites removed, please use a larger slab.",
                        stacklevel=2,
                    )
                    break
...
def get_d(slab: Slab) -> float:
    ...
    for site, next_site in itertools.pairwise(sorted_sites):
        if not math.isclose(
            site.frac_coords[2], next_site.frac_coords[2], abs_tol=1e-6
        ):
...
class ReconstructionGenerator:
    ...
    def __init__(
        ...
        def build_recon_json() -> dict:
            ...
            recon_json: dict = copy.deepcopy(
                RECONSTRUCTIONS_ARCHIVE[reconstruction_name]
            )
    ...
    def build_slabs(self) -> list[Slab]:
        ...
        for slab in slabs:
            ...
            if "points_to_remove" in self.reconstruction_json:
                sites_to_rm: list = copy.deepcopy(
                    self.reconstruction_json["points_to_remove"]
                )
                for site in sites_to_rm:
                    ...
                    distances: list[float] = [
                        site.distance_from_point(cart_point) for site in slab
                    ]
            ...
            if "points_to_add" in self.reconstruction_json:
                sites_to_add: list = copy.deepcopy(
                    self.reconstruction_json["points_to_add"]
                )
    ...
    for miller in itertools.product(idx_range, idx_range, idx_range):
        ...
        if any(idx != 0 for idx in miller):
            ...
            if (
                ...
                and _is_in_miller_family(
                    max_idx * np.array(miller), equivalent_millers, symm_ops
                )
            ):
                equivalent_millers += [miller]
...
def hkl_transformation(
    ...
    reduced_transf = (
        reduce(math_lcm, [int(1 / i) for i in itertools.chain(*transf) if i != 0])
        * transf
    )
