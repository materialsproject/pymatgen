...
__credits__ = (
    "Maarten de Jong, Shyam Dwaraknath, Wei Chen, Mark Asta, Anubhav Jain, Terence Lew"
)
...
class Tensor(np.ndarray, MSONable):
    ...
    def is_voigt_symmetric(self, tol: float = 1e-6) -> bool:
        ...
        transpose_pieces += [
            [list(range(j, j + 2))] for j in range(self.rank % 2, self.rank, 2)
        ]
    ...
    def get_ieee_rotation(
        ...
        # IEEE rules: a=b in length; c,a || x3, x1
            ...
            rotation = np.array(
                [
                    vec / mag
                    for (mag, vec) in sorted(
                        zip(lengths, vecs, strict=True), key=lambda x: x[0]
                    )
                ]
            )
        ...
        # IEEE rules: c<a<b; c,a || x3,x1
            ...
            rotation = np.roll(
                [vec / mag for (mag, vec) in sorted(zip(lengths, vecs, strict=True))],
                2,
                axis=0,
            )
        ...
        # IEEE rules: b,c || x2,x3; alpha=beta=90, c<a
            ...
            c = next(
                vec / mag
                for (mag, vec) in sorted(
                    zip(lengths[n_umask], vecs[n_umask], strict=True)
                )
            )
...
class TensorCollection(collections.abc.Sequence, MSONable):
    ...
    def __init__(self, tensor_list: Sequence, base_class=Tensor) -> None:
        ...
        """
        self.tensors = [
            tensor if isinstance(tensor, base_class) else base_class(tensor)
    ...
    def fit_to_structure(
        ...
        """
        return type(self)(
            [tensor.fit_to_structure(structure, symprec) for tensor in self]
        )
    ...
    def convert_to_ieee(
        ...
        return type(self)(
            [
                tensor.convert_to_ieee(structure, initial_fit, refine_rotation)
                for tensor in self
            ]
        )
...
class SquareTensor(Tensor):
    ...
    def is_rotation(
        ...
        if include_improper:
            det = np.abs(det)
        return np.allclose(self.inv, self.trans, atol=tol, rtol=0) and np.allclose(
            det, 1.0, atol=tol, rtol=0
        )
