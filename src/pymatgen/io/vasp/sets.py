...
class VaspInputSet(InputGenerator, abc.ABC):
    ...
    def __post_init__(self) -> None:
        ...
        if (
            valid_potcars := self._valid_potcars
        ) and user_potcar_functional not in valid_potcars:
            raise ValueError(
                f"Invalid {user_potcar_functional=}, must be one of {valid_potcars}"
            )
        ...
        self.user_potcar_functional: UserPotcarFunctional = (
            self.user_potcar_functional
            or self._config_dict.get("POTCAR_FUNCTIONAL", "PBE")
        )

        # Warn if a user is overriding POTCAR_FUNCTIONAL
        if self.user_potcar_functional != self._config_dict.get(
    ...
    def structure(self, structure: Structure | None) -> None:
        ...
        if isinstance(structure, SiteCollection):  # could be Structure or Molecule
            ...
            struct_has_Yb = any(
                specie.symbol == "Yb" for site in structure for specie in site.species
            )
    ...
    def incar(self) -> Incar:
        ...
        if self.inherit_incar is True and self.prev_incar:
            ...
            prev_incar = {
                k: cast("dict[str, Any]", self.prev_incar)[k]
                for k in self.inherit_incar
                if k in self.prev_incar
            }
        ...
        for key, setting in settings.items():
            if key == "MAGMOM":
                ...
                for site in structure:
                    if uic_magmom := self.user_incar_settings.get("MAGMOM", {}).get(
                        site.species_string
                    ):
                        ...
                        if (
                            site.specie.symbol == "Co"
                            and setting[str(site.specie)] <= 1.0
                        ):
                            warnings.warn(
                ...
                if hubbard_u:
                    if hasattr(structure[0], key.lower()):
                    ...
                    elif most_electro_neg in setting and isinstance(
                        setting[most_electro_neg], dict
                    ):
                        incar[key] = [
                            setting[most_electro_neg].get(sym, 0)
                        ...
                        incar[key] = [
                            (
                                setting.get(sym, 0)
                                if isinstance(setting.get(sym, 0), float | int)
                                else 0
                            )
                            for sym in poscar.site_symbols
                        ]
        ...
        _apply_incar_updates(
            incar, self.user_incar_settings, skip=["LDAUU", "LDAUJ", "LDAUL", "MAGMOM"]
        )
        ...
        if kpoints is not None:
            ...
            "KSPACING" in incar
            and "KSPACING" not in self.user_incar_settings
            and "KSPACING" in prev_incar
        ):
    ...
    def nelect(self) -> float:
        ...
        n_elect = sum(
            num_atoms * n_electrons_by_element[el.symbol]
            for el, num_atoms in self.structure.composition.items()
        )
    ...
    def kpoints(self) -> Kpoints | None:
        ...
        if "line_density" in kconfig and zero_weighted_kpoints:
            raise ValueError(
                "Cannot combine line_density and zero weighted k-points options"
            )
        if zero_weighted_kpoints and not base_kpoints:
            raise ValueError(
                "Zero weighted k-points must be used with reciprocal_density or grid_density options"
            )
    ...
    def override_from_prev_calc(self, prev_calc_dir: PathLike = ".") -> Self:
        ...
        files_to_transfer = {}
        if getattr(self, "copy_chgcar", False) and (
        ...
        if getattr(self, "copy_wavecar", False):
            for fname in ("WAVECAR", "WAVEDER", "WFULL"):
                if wavecar_files := sorted(
                    glob(str(Path(prev_calc_dir) / (f"{fname}*")))
                ):
                    if fname == "WFULL":
    ...
    def calculate_ng(
        ...
        if custom_encut is not None:
            ...
            encut = max(
                i_species.enmax for i_species in self.get_vasp_input()["POTCAR"]
            )
        ...
        if PREC[0].lower() not in {"a", "s", "n", "l", "m", "h"}:
            raise ValueError(
                f"{PREC=} does not exist. If this is no longer correct, please update this code."
            )

        CUTOFF = [
            np.sqrt(encut / _RYTOEV) / (2 * np.pi / (anorm / _AUTOA))
    ...
    @staticmethod
    def from_directory(
        directory: PathLike, optional_files: dict | None = None
    ) -> VaspInput:
...
class MP24RelaxSet(VaspInputSet):
    ...
    def __post_init__(self) -> None:
        ...
        if self.dispersion == "rVV10":
            ...
            config_updates |= {
                "IVDW": 13,
                **{f"VDW_{k}": v for k, v in d4_pars[self.xc_functional].items()},
            }
...
class MPStaticSet(VaspInputSet):
    ...
    def kpoints_updates(self) -> dict | Kpoints:
        ...
        if (
            ...
            k_div = cast(
                "Kpoint", tuple(kp + 1 if kp % 2 == 1 else kp for kp in kpoints.kpts[0])
            )
...
class MatPESStaticSet(VaspInputSet):
    ...
    def __post_init__(self) -> None:
        ...
        default_potcars = (
            self.CONFIG["PARENT"].replace("PBE", "PBE_").replace("Base", "")
        )  # PBE64Base -> PBE_64
...
class MPHSEBSSet(VaspInputSet):
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        if self.prev_vasprun is not None:
            # Set NBANDS
            nbands = int(
                np.ceil(self.prev_vasprun.parameters["NBANDS"] * self.nbands_factor)
            )
...
class MPNonSCFSet(VaspInputSet):
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        # Automatic setting of NEDOS using the energy range and the energy step
        nedos = (
            _get_nedos(self.prev_vasprun, self.dedos) if self.nedos == 0 else self.nedos
        )
    ...
    def structure(self, structure: Structure | None) -> None:
        if structure is not None:
            ...
            if not hasattr(structure[0], "magmom"):
                raise ValueError(
                    "Neither the previous structure has magmom property nor magmom provided"
                )
...
class MVLGWSet(VaspInputSet):
    ...
    def __post_init__(self) -> None:
        ...
        if mode not in MVLGWSet.SUPPORTED_MODES:
            raise ValueError(
                f"Invalid {mode=}, supported modes are {', '.join(map(repr, MVLGWSet.SUPPORTED_MODES))}"
            )
    ...
    @classmethod
    def from_prev_calc(
        cls, prev_calc_dir: PathLike, mode: str = "DIAG", **kwargs
    ) -> Self:
...
class MVLSlabSet(VaspInputSet):
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        if self.structure is not None:
            ...
            if self.structure.site_properties and self.structure.site_properties.get(
                "surface_properties"
            ):
                non_adsorbate_elts = {
                    s.specie.symbol
        ...
        if not self.bulk:
            ...
            if self.auto_dipole and self.structure is not None:
                ...
                center_of_mass = np.average(
                    self.structure.frac_coords, weights=weights, axis=0
                ).tolist()
...
class NEBSet(VaspInputSet):
    ...
    def __init__(
        ...
        unset_encut: bool = False,
        parent_set="MPRelaxSet",
        **kwargs,
    ) -> None:
        """
        Args:
        ...
        if len(structures) < 3:
            raise ValueError(
                f"You need at least 3 structures for an NEB, got {len(structures)}"
            )
...
class MVLNPTMDSet(VaspInputSet):
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        enmax = [
            self.potcar[i].keywords["ENMAX"] for i in range(self.structure.n_elems)
        ]
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        if self.user_supplied_basis is None and self.address_basis_file is None:
            basis = Lobsterin.get_basis(
                structure=self.structure, potcar_symbols=potcar_symbols
            ...
            for atom_type in self.structure.symbol_set:  # type: ignore[union-attr]
                if atom_type not in self.user_supplied_basis:
                    raise ValueError(
                        f"There are no basis functions for the atom type {atom_type}"
                    )
            basis = [
                f"{key} {value}" for key, value in self.user_supplied_basis.items()
            ]
        else:
            basis = None
...
def get_vasprun_outcar(
    ...
    if not vruns or not outcars:
        raise ValueError(
            f"Unable to get vasprun.xml/OUTCAR from prev calculation in {path}"
        )
...
def get_structure_from_prev_run(
    vasprun: Vasprun, outcar: Outcar | None = None
) -> Structure:
...
def standardize_structure(
    ...
    if abs(vpa_old - vpa_new) / vpa_old > 0.02:
        raise ValueError(
            f"Standardizing cell failed! VPA old: {vpa_old}, VPA new: {vpa_new}"
        )
...
class MPAbsorptionSet(VaspInputSet):
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode not in type(self).SUPPORTED_MODES:
            raise ValueError(
                f"{self.mode} not one of the support modes : {type(self).SUPPORTED_MODES}"
            )
    ...
    def incar_updates(self) -> dict[str, Any]:
        ...
        if self.prev_vasprun is not None and self.mode == "RPA":
            ...
            # other lattices.
            self.nkred = (
                cast("tuple[int, int, int]", self.prev_vasprun.kpoints.kpts[0])
                if self.nkred is None
                else self.nkred
            )
            updates |= {
                "NKREDX": self.nkred[0],
...
def _apply_incar_updates(
    incar: dict | Incar, updates: dict[str, Any], skip: Sequence[str] = ()
) -> None:
    """
