...
class Outcar:
    ...
    def __init__(self, filename: PathLike) -> None:
        ...
        # Read total elastic tensor if FD (ionic) and variable cell (electronic)
        if self.fd and self.varcell:
            self.read_elastic_tensor()
        # Check if LCALCEPS is True and read piezo data if so
        self.read_pattern({"calceps": r"LCALCEPS\s*=\s*T"})
        if self.data.get("calceps", False):
            self.lcalceps = True
            # self.read_lcalceps()
            self.read_lepsilon()
            # Only read ionic contribution if DFPT is turned on
    ...
    def read_table_pattern(
        ...
        # print("!!!!!I'M IN read_table_pattern()!!!!!") #TEST
        ...
        # print(f"....table_pattern: {table_pattern}!!!!!") #TEST
        # print(f"rp: {rp}!!!!!") #TEST
        tables: list[list] = []
        for mt in table_pattern.finditer(text):
            table_body_text = mt.group("table_body")
            # print(f".....table_body_text is: {table_body_text}") #TEST
            table_contents = []
            ...
            # print("....APPENDED TABLE!!!!!") #TEST
            if first_one_only:
                break
        # print(f".....length of tables is: {len(tables)}") #TEST
        # print(f".....attribute name is: {attribute_name}") #TEST
        retained_data: list = tables[-1] if last_one_only or first_one_only else tables
        # print("....RETAINED DATA!!!!!") #TEST
