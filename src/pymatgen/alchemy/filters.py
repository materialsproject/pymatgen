            ...
            if sp_to_test := species.intersection(all_species):
                ...
                for sp in sp_to_test:
                    for nn_site, dist, *_ in neighbors:
                        if (
                            sp in nn_site.species
                            and dist < self.specie_and_min_dist[sp]
                        ):
                            return False
        return True
...
class RemoveDuplicatesFilter(AbstractStructureFilter):
    ...
    def __init__(
        ...
        if not isinstance(structure_matcher, dict | StructureMatcher | type(None)):
            raise TypeError(
                f"{structure_matcher=} must be a dict, StructureMatcher or None"
            )
    ...
    def test(self, structure: Structure) -> bool:
        ...
        for struct in self.structure_list[hash_comp]:
            if (
                self.symprec is None or get_spg_num(struct) == get_spg_num(structure)
...
class SpeciesMaxDistFilter(AbstractStructureFilter):
    ...
    def test(self, structure: Structure):
        ...
        sp1_indices = [
            idx for idx, site in enumerate(structure) if site.specie == self.sp1
        ]
        sp2_indices = [
            idx for idx, site in enumerate(structure) if site.specie == self.sp2
        ]
