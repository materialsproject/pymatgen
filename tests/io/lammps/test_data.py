...
-2.4567 0.0000 0.0000  xy xz yz"""
    ...
    def test_get_box_shift(self):
        ...
        assert_allclose(
            quartz.get_box_shift([1, -1, 0]), [4.9134 + 2.4567, -4.2551, 0], 4
        )
...
class TestLammpsData(MatSciTest):
    ...
    def setup_class(cls):
        ...
        cls.quartz = LammpsData.from_file(
            filename=f"{TEST_DIR}/data.quartz", atom_style="atomic"
        )
        cls.virus = LammpsData.from_file(
            filename=f"{TEST_DIR}/virus.data", atom_style="angle"
        )
        cls.tatb = LammpsData.from_file(
            filename=f"{TEST_DIR}/tatb.data", atom_style="charge", sort_id=True
        )
    ...
    def test_structure(self):
        ...
        assert_allclose(ethane.lattice.matrix, np.diag([10.0] * 3))
        l_bounds = np.array(self.ethane.box.bounds)[:, 0]
        coords = self.ethane.atoms[["x", "y", "z"]] - l_bounds
        assert_allclose(ethane.cart_coords, coords)
        assert_allclose(ethane.site_properties["charge"], self.ethane.atoms["q"])
        tatb = self.tatb.structure
        frac_coords = tatb.frac_coords[381]
        real_frac_coords = frac_coords - np.floor(frac_coords)
        assert_allclose(
            real_frac_coords, [0.01553397, 0.71487872, 0.14134139], atol=1e-8
        )

        co = Structure.from_spacegroup(
            194, Lattice.hexagonal(2.50078, 4.03333), ["Co"], [[1 / 3, 2 / 3, 1 / 4]]
        )
...
"""
        ...
        kw_inds = {
            line: idx for idx, line in enumerate(quartz_lines) if line in quartz_kws
        }
        # header
...
"""
    ...
    def test_write_file(self):
        ...
        pd.testing.assert_frame_equal(
            c2h6.force_field[ff_kw], self.ethane.force_field[ff_kw], ff_kw
        )
        topo_kw = rng.choice(sorted(self.ethane.topology), 1)[0]
        pd.testing.assert_frame_equal(
            c2h6.topology[topo_kw], self.ethane.topology[topo_kw], topo_kw
        )
        ...
        pd.testing.assert_frame_equal(
            v.force_field["PairIJ Coeffs"], self.virus.force_field["PairIJ Coeffs"]
        )
    ...
    def test_disassemble(self):
        ...
        for kw in base_kws:
            ...
            assert_allclose(
                sample_coeff["coeffs"],
                c.force_field[ff_kw].iloc[idx].values,
                err_msg=ff_kw,
            )
        topo = topos[-1]
        ...
        assert topo.sites.site_properties["ff_map"] == [
        ...
        for kw in base_kws:
            ...
            sample_topo = rng.choice(
                list(topo_df.itertuples(index=False, name=None)), 1
            )[0]
            topo_type_idx = sample_topo[0] - 1
            topo_type = tuple(
                atom_labels[i - 1] for i in atoms.loc[list(sample_topo[1:])]["type"]
            )
    ...
    def test_from_ff_and_topologies(self):
        ...
        ld_wo_angles = LammpsData.from_ff_and_topologies(
    ...
    def test_from_structure(self):
        ...
        lammps_data = LammpsData.from_structure(
            structure=structure, ff_elements=["O", "Os", "Na"]
        )
    ...
    def test_json_dict(self):
        ...
        assert (
            pd.testing.assert_frame_equal(
                c2h6.force_field[key], target_df, check_dtype=False
            )
            is None
        ), key
...
class TestTopology:
    def test_init(self):
        ...
        topo_override = Topology(
            sites=mol, ff_label="ff_map", charges=outer_charge, velocities=outer_velo
        )
    ...
    def test_from_bonding(self):
        ...
        # bond flag to off
        topo_etoh0 = Topology.from_bonding(
            molecule=etoh, bond=False, angle=True, dihedral=True
        )
...
class TestForceField(MatSciTest):
    ...
    def setup_class(cls):
        ...
        cls.virus = ForceField(
            mass_info=mass_info, nonbond_coeffs=non_bond_coeffs, topo_coeffs=topo_coeffs
        )
...
class TestFunc:
    def test_lattice_2_lmpbox(self):
        rng = np.random.default_rng()
        matrix = (
            np.diag(rng.integers(5, 14, size=(3,))) + rng.random((3, 3)) * 0.2 - 0.1
        )
        ...
        boxed_structure = Structure(
            boxed_latt, ["H"] * 10, cart_coords, coords_are_cartesian=True
        )
...
class TestCombinedData:
    ...
    def setup_class(cls):
        ...
        cls.ec_fec2 = CombinedData.from_lammpsdata(
            [cls.ec, cls.fec], ["EC", "FEC"], [1200, 300], cls.coord
        )
        ...
        cls.ec_fec3 = CombinedData.from_lammpsdata(
            [cls.ec_fec_ld], ["EC FEC"], [2], cls.double_coord
        )
        cls.li_ec = CombinedData.from_lammpsdata(
            [cls.li, cls.ec], ["Li", "EC"], [1, 1], cls.small_coord
        )
        cls.ec_li = CombinedData.from_lammpsdata(
            [cls.ec, cls.li], ["EC", "Li"], [1, 1], cls.small_coord_2
        )
        cls.li_2 = CombinedData.from_lammpsdata(
            [cls.li], ["Li"], [2], cls.small_coord_3
        )
        cls.li_2_minimal = CombinedData.from_lammpsdata(
            [cls.li_minimal], ["Li_minimal"], [2], cls.small_coord_3
        )
    ...
    def test_from_lammpsdata(self):
        ...
        for key in ("Bonds", "Angles", "Dihedrals", "Impropers"):
            pd.testing.assert_frame_equal(
                ec_li_minimal.topology[key], ec_li_minimal.topology[key]
            )
        pd.testing.assert_frame_equal(
        ...
        for key in (
            ...
            pd.testing.assert_frame_equal(
                ec_li_minimal.force_field[key], ec_li.force_field[key]
            )
    ...
    def test_get_str(self):
        ...
        assert ec_fec_double_lines[30146].endswith(
            "12 -0.2329  4.630985  7.328547 51.604678"
        )
    ...
    def test_structure(self):
        ...
        assert_allclose(li_ec_structure.cart_coords, coords)
        assert_allclose(
            li_ec_structure.site_properties["charge"], self.li_ec.atoms["q"]
        )
        frac_coords = li_ec_structure.frac_coords[0]
        real_frac_coords = frac_coords - np.floor(frac_coords)
        assert_allclose(
            real_frac_coords, [0.01292047, 0.01292047, 0.01292047], atol=1e-8
        )

    def test_from_ff_and_topologies(self):
        with pytest.raises(
            AttributeError, match="Unsupported constructor for CombinedData objects"
        ):
    ...
    def test_json_dict(self):
        ...
        assert (
            pd.testing.assert_frame_equal(
                lic3o3h4.force_field[key], target_df, check_dtype=False
            )
            is None
        ), key
        ...
        key, target_df = topo_items[rng.choice(len(topo_items))]
        assert (
            pd.testing.assert_frame_equal(lic3o3h4.topology[key], target_df) is None
        ), key
        ...
        pd.testing.assert_frame_equal(
            lic3o3h4.mols[1].masses, self.li_ec.mols[1].masses
        )
        ...
        lic3o3h4.mols[1].force_field[key].index = (
            lic3o3h4.mols[1].force_field[key].index.map(int)
        )
        assert (
            pd.testing.assert_frame_equal(
                lic3o3h4.mols[1].force_field[key], target_df, check_dtype=False
            )
            is None
        ), key
        ...
        lic3o3h4.mols[1].topology[key].index = (
            lic3o3h4.mols[1].topology[key].index.map(int)
        )
        assert (
            pd.testing.assert_frame_equal(lic3o3h4.mols[1].topology[key], target_df)
            is None
        ), key
