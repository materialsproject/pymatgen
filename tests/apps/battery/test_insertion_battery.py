...
class TestInsertionElectrode(unittest.TestCase):
    def setUp(self):
        ...
        with open(os.path.join(TEST_FILES_DIR, "LiTiO2_batt.json")) as f:
            self.entries_LTO = json.load(f, cls=MontyDecoder)

        with open(os.path.join(TEST_FILES_DIR, "MgVO_batt.json")) as file:
            self.entries_MVO = json.load(file, cls=MontyDecoder)

        with open(os.path.join(TEST_FILES_DIR, "Mg_batt.json")) as file:
            self.entry_Mg = json.load(file, cls=MontyDecoder)

        with open(os.path.join(TEST_FILES_DIR, "CaMoO2_batt.json")) as f:
