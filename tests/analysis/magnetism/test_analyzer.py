...
class TestCollinearMagneticStructureAnalyzer:
    def setup_method(self):
        ...
        self.LiFePO4 = Structure.from_file(
            f"{TEST_FILES_DIR}/cif/LiFePO4.cif", primitive=True
        )

        self.Fe3O4 = Structure.from_file(
            f"{TEST_FILES_DIR}/cif/Fe3O4.cif", primitive=True
        )
        ...
        self.NiO_expt = Structure.from_file(
            f"{TEST_FILES_DIR}/io/cif/mcif/magnetic.example.NiO.mcif", primitive=True
        )
        ...
        self.NiO_AFM_111 = Structure(
            lattice, species, coords, site_properties={"magmom": [-5, 5, 0, 0]}
        )

        lattice = Lattice([[2.085, 2.085, 0], [0, 0, -4.17], [-2.085, 2.085, 0]])
        species = ["Ni", "Ni", "O", "O"]
        coords = [[0.5, 0.5, 0.5], [0, 0, 0], [0, 0.5, 0], [0.5, 0, 0.5]]
        self.NiO_AFM_001 = Structure(
            lattice, species, coords, site_properties={"magmom": [-5, 5, 0, 0]}
        )
        ...
        self.NiO_unphysical = Structure(
            lattice, species, coords, site_properties={"magmom": [-3, 0, 0, 0]}
        )
    ...
    def test_modes(self):
        ...
        mode = "respect_sign"
        msa = CollinearMagneticStructureAnalyzer(
            self.NiO_unphysical, overwrite_magmom_mode=mode
        )
        magmoms = msa.structure.site_properties["magmom"]
        ...
        mode = "replace_all"
        msa = CollinearMagneticStructureAnalyzer(
            self.NiO_unphysical, overwrite_magmom_mode=mode, make_primitive=False
        )
        magmoms = msa.structure.site_properties["magmom"]
        ...
        with pytest.raises(
            ValueError, match="'invalid_mode' is not a valid OverwriteMagmomMode"
        ):
            CollinearMagneticStructureAnalyzer(
                self.NiO, overwrite_magmom_mode="invalid_mode"
            )
    ...
    def test_magnetic_properties(self):
        ...
        for threshold, expected in [(1e-8, Ordering.AFM), (1e-20, Ordering.FiM)]:
            mag_struct_analyzer = CollinearMagneticStructureAnalyzer(
                self.CuO_expt, threshold_ordering=threshold
            )
...
-5.00   PeriodicSite: Ni (0.0, 2.085, -2.085) [0.5, 0.5, 0.5]"""
    ...
    def test_round_magmoms(self):
        ...
        msa = CollinearMagneticStructureAnalyzer(
            struct, round_magmoms=0.001, make_primitive=False
        )
...
class TestMagneticStructureEnumerator:
    ...
    def test_default_transformation_kwargs(self):
        ...
        enumerator = MagneticStructureEnumerator(
            structure, transformation_kwargs=transformation_kwargs
        )
...
class TestMagneticDeformation:
    def test_magnetic_deformation(self):
        test_structs = loadfn(
            f"{TEST_FILES_DIR}/analysis/magnetism/magnetic_deformation.json"
        )
