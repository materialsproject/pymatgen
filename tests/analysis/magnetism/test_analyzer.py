...
class TestCollinearMagneticStructureAnalyzer(unittest.TestCase):
    def setUp(self):
        parser = CifParser(os.path.join(TEST_FILES_DIR, "Fe.cif"))
        ...
        parser = CifParser(os.path.join(TEST_FILES_DIR, "magnetic.ncl.example.GdB4.mcif"))
        self.GdB4 = parser.get_structures()[0]

        parser = CifParser(os.path.join(TEST_FILES_DIR, "magnetic.example.NiO.mcif"))
...
class TestMagneticStructureEnumerator(unittest.TestCase):
    ...
    def test_ordering_enumeration(self):
        # simple afm
        structure = Structure.from_file(os.path.join(TEST_FILES_DIR, "magnetic_orderings/LaMnO3.json"))
        enumerator = MagneticStructureEnumerator(structure)
        assert enumerator.input_origin == "afm"

        # ferrimagnetic (Cr produces net spin)
        structure = Structure.from_file(os.path.join(TEST_FILES_DIR, "magnetic_orderings/Cr2NiO4.json"))
        enumerator = MagneticStructureEnumerator(structure)
        assert enumerator.input_origin == "ferri_by_Cr"

        # antiferromagnetic on single magnetic site
        structure = Structure.from_file(os.path.join(TEST_FILES_DIR, "magnetic_orderings/Cr2WO6.json"))
        enumerator = MagneticStructureEnumerator(structure)
        ...
        structure = Structure.from_file(os.path.join(TEST_FILES_DIR, "magnetic_orderings/Ca3Co2O6.json"))
...
class TestMagneticDeformation(unittest.TestCase):
    def test_magnetic_deformation(self):
        test_structs = loadfn(os.path.join(TEST_FILES_DIR, "magnetic_deformation.json"))
