...
class TestWulffShape(MatSciTest):
    ...
    def test_properties(self):
        ...
        for mp_id, wulff in wulff_shapes.items():
            ...
            assert round(wulff.weighted_surface_energy, 3) == round(
                properties["weighted_surface_energy"], 3
            )
            assert round(wulff.shape_factor, 3) == round(properties["shape_factor"], 3)
            assert round(wulff.anisotropy, 3) == round(
                properties["surface_anisotropy"], 3
            )
