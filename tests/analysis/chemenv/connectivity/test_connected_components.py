...
class TestConnectedComponent(MatSciTest):
    ...
    def test_serialization(self):
        ...
        en1 = EnvironmentNode(
            central_site=PeriodicSite(
                "Si", coords=np.array([0.0, 0.0, 0.0]), lattice=lattice
            ),
            i_central_site=3,
            ce_symbol="T:4",
        )
        en2 = EnvironmentNode(
            central_site=PeriodicSite(
                "Ag", coords=np.array([0.0, 0.0, 0.5]), lattice=lattice
            ),
            i_central_site=5,
            ce_symbol="T:4",
        )
        en3 = EnvironmentNode(
            central_site=PeriodicSite(
                "Ag", coords=np.array([0.0, 0.5, 0.5]), lattice=lattice
            ),
            i_central_site=8,
            ce_symbol="O:6",
        )
        ...
        cc_from_json = ConnectedComponent.from_dict(
            orjson.loads(orjson.dumps(cc.as_dict()).decode())
        )
        ...
        for loaded_cc in loaded_cc_list:
            ...
            assert sorted_edges == sorted(
                sorted(edge) for edge in loaded_cc.graph.edges()
            )
    ...
    def test_periodicity(self):
        env_node1 = EnvironmentNode(
            central_site="Si", i_central_site=3, ce_symbol="T:4"
        )
        env_node2 = EnvironmentNode(
            central_site="Ag", i_central_site=5, ce_symbol="T:4"
        )
        env_node3 = EnvironmentNode(
            central_site="Ag", i_central_site=8, ce_symbol="O:6"
        )
        env_node4 = EnvironmentNode(
            central_site="Fe", i_central_site=23, ce_symbol="C:8"
        )
