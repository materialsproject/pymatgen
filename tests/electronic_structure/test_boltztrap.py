...
class TestBoltztrapAnalyzer(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.bz = BoltztrapAnalyzer.from_files(os.path.join(TEST_FILES_DIR, "boltztrap/transp/"))
        cls.bz_bands = BoltztrapAnalyzer.from_files(os.path.join(TEST_FILES_DIR, "boltztrap/bands/"))
        cls.bz_up = BoltztrapAnalyzer.from_files(os.path.join(TEST_FILES_DIR, "boltztrap/dos_up/"), dos_spin=1)
        cls.bz_dw = BoltztrapAnalyzer.from_files(os.path.join(TEST_FILES_DIR, "boltztrap/dos_dw/"), dos_spin=-1)
        cls.bz_fermi = BoltztrapAnalyzer.from_files(os.path.join(TEST_FILES_DIR, "boltztrap/fermi/"))
    ...
    def test_get_symm_bands(self):
        structure = loadfn(os.path.join(TEST_FILES_DIR, "boltztrap/structure_mp-12103.json"))
        sbs = loadfn(os.path.join(TEST_FILES_DIR, "boltztrap/dft_bs_sym_line.json"))
