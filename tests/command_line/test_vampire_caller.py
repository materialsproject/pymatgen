...
@pytest.mark.skipif(
    not which("vampire-serial"), reason="vampire executable not present"
)
class TestVampireCaller(MatSciTest):
    ...
    def setup_class(cls):
        ...
        for compound in cls.compounds:
            ...
            energies = [
                e * len(s)
                for (e, s) in zip(e_per_atom, ordered_structures, strict=True)
            ]
    ...
    def test_vampire(self):
        for structs, energies in zip(
            self.structure_inputs, self.energy_inputs, strict=True
        ):
