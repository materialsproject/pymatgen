...
@pytest.mark.skipif(
    not (which("mcsqs") and which("str2cif")), reason="mcsqs executable not present"
)
class TestMcsqsCaller(MatSciTest):
    ...
    def test_mcsqs_caller_supercell(self):
        ...
        sqs = run_mcsqs(
            struct, {2: 6, 3: 4}, scaling=[2, 1, 1], search_time=0.01, instances=1
        )
    ...
    def test_mcsqs_caller_total_atoms_auto_instances(self):
        ...
        sqs = run_mcsqs(
            struct, {2: 6, 3: 4}, scaling=2, search_time=0.01, instances=None
        )
