...
def test_pmg_analyze(cd_tmp_path: Path):
    exit_status = os.system(f"pmg analyze {TEST_FILES_DIR}/io/vasp/fixtures/scan_relaxation")  # noqa:S605
    assert exit_status == 0
    assert os.path.isfile("vasp_data.gz")


def test_pmg_structure(cd_tmp_path: Path):
    exit_status = os.system(f"pmg structure --convert --filenames {TEST_FILES_DIR}/cif/Li2O.cif POSCAR_Li2O_test")  # noqa:S605
    assert exit_status == 0
    ...
    exit_status = os.system(f"pmg structure --localenv Li-O=3 --filenames {TEST_FILES_DIR}/cif/Li2O.cif")  # noqa:S605
    assert exit_status == 0


def test_pmg_diff(cd_tmp_path: Path):
    exit_status = os.system(f"pmg diff --incar {VASP_IN_DIR}/INCAR {VASP_IN_DIR}/INCAR_2")  # noqa:S605
    assert exit_status == 0
